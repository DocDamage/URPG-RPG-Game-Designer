...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "usage: writeback_localization <dialogue_preview.json> <bundle.json> <output.json>\n";
        return 1;
    }

    try {
        writeJson(argv[3],
                  urpg::localization::writebackDialoguePreviewLocalizationBundle(readJson(argv[1]), readJson(argv[2])));
