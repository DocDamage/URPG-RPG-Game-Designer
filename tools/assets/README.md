...
RAR/7z import is deliberately selected-entry only. Configure `URPG_ASSET_ARCHIVE_EXTRACTOR` with the source, destination, and a standalone selected-entry placeholder, for example `7z x -y -o{destination} {source} {selected_entries}`. The editor passes only checked, safety-reviewed archive paths to `{selected_entries}`; a command without that placeholder is allowed for read-only listing but is refused for import.

## Native editor catalog interchange

After indexing, publish the SQLite metadata as the bounded interchange consumed by the native Assets workspace:

```powershell
python .\tools\assets\catalog_interchange.py --db .urpg/asset-index/asset_catalog.db
```

This atomically replaces `.urpg/asset-index/catalog_meta.json` after all capped JSONL shards are ready. The output contains virtual paths, discovery metadata, tags, scan/root state, and hash-pending counts only; it never copies or serializes asset payloads. If the editor reports an incompatible catalog schema, rerun the command above after updating the tooling.
