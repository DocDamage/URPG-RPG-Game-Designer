...
$matrix = Get-Content -LiteralPath $resolvedMatrix -Raw | ConvertFrom-Json -Depth 20
...
foreach ($document in $documents) {
    ...
    foreach ($field in @("documentType", "owner", "pathPattern", "saveOwner", "historyStatus", "migrationStatus", "referenceStatus")) {
...
Write-Host "Document ownership matrix passed: $($documents.Count) durable native documents have explicit owner, persistence, history, recovery, migration, reference, and gap status."
