...
`AssetTransformRevisionService` also implements `image_palette_extract`: it counts exact decoded RGBA values, ranks them by frequency descending and RGBA ascending tie break, selects up to the requested two-to-256 color limit, then applies the same deterministic nearest-color reduction. The requested limit, selected ordered palette, and selection policy are all part of immutable revision identity and manifest. It reuses matching complete output, rejects one-color images and invalid limits before publication, and the single PNG is accepted by the existing reviewed derived-revision attachment path. This is exact-color extraction only; clustering, color-profile conversion, dithering, visual palette editing, and perceptual color-space selection remain open.
