...
| M7 | Initial implementation | Checksummed recovery snapshot primitives, orderly-session markers, five-minute bounded private capture of dirty Grid Parts, Perspective 2D, and ability drafts, plus an in-editor count and safe restore-to-new-folder action are implemented. Guided recovery-project review/open flow, broader durable-surface coverage, and asset relinking remain open. |
