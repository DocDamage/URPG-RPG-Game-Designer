...
| M7 | Initial implementation | Checksummed recovery snapshot primitives, orderly-session markers, five-minute bounded private capture of dirty Grid Parts, Perspective 2D, ability, Character Creator, quest, database, vendor, audio-mix, and input-remap drafts, plus in-editor count, safe restore-to-new-folder, and clean-session open-recovered-project actions are implemented. Broader durable-surface coverage and asset relinking remain open. |
| M8 | Initial implementation | Map can open the active project's durable Character Creator draft or Ability Inspector while preserving project/Map context and shared save, navigation, and recovery ownership. On project bind it loads one authored ability into the native preview runtime, preferring `willow_strike` for the governed vertical-slice context. Its Perspective 2D event form creates an event, first page, and a supported native command including text/choice dialogue, transfer, inventory, encounter launch, and vendor open in the normal Map document; a successful dialogue event binds its actual native runtime output to the Message Inspector, and its event runtime feeds switch, variable, inventory, and dialogue-choice state into the contextual quest preview. An authored encounter-launch event opens the native Battle Inspector with the active character and ability draft queued and a vendor-open command resolves the native vendor stock preview by stable ID. A marked Battle Inspector preview victory feeds the quest preview only and is explicitly not a playtest combat claim. Its contextual audio control applies and saves a project mix preset through the native audio core and assigns an attached audio asset with owner-local undo/redo; its contextual input control edits, saves, recovers, and restores defaults through the versioned native input-remap store; its contextual accessibility control audits the active audio-mix and battle-preview surfaces through the native auditor; its contextual export diagnostics control runs project-local DevBootstrap staging preflight without emitting a package; its quest form creates, previews, saves, and recovers a typed objective graph; and its database/vendor form creates project items and database-validated vendor stock. Battle-result, broader audio cue routing, and WYSIWYG breadth remain open. |
| M9 | Draft seeded | The governed vertical-slice scenario has a native-wizard draft bundle with two maps, Perspective 2D events, character, ability, quest, database item, and vendor stock; contextual native export preflight; and Map-context native save/load of the exact Perspective 2D preview state (with explicit preview-only battle outcomes). It remains a draft: no creator-authored passed runtime/package report exists. |
