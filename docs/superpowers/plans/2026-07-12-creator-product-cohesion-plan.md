...
| M6 | Initial implementation | Editor-owned process lifecycle, private current-map overlay, authored player-start or selected-part target (with blank-starter fallback), visible target/elapsed/exit/overlay state, F5 stop/restart, document/catalog launch blocking, and bounded JSONL diagnostics are implemented. Broader runtime blocker policy, map-entrance/checkpoint targets, diagnostic focus, and safe reload remain open. |
