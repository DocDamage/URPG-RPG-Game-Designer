...
| M9 | Draft seeded | The governed vertical-slice scenario has a native-wizard draft bundle with two maps, Perspective 2D events, character, ability, quest, database item, and vendor stock; a contextual native export-preflight route; and a strict report gate. It remains a draft: no creator-authored passed runtime/package report exists. |
