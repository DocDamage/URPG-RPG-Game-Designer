...
| M6 | Initial implementation | Editor-owned process lifecycle, private current-map overlay, authored player-start or selected-part target (with blank-starter fallback), F5 stop/restart, and bounded JSONL diagnostics are implemented. Blocker enforcement, map-entrance/checkpoint targets, diagnostic focus, and safe reload remain open. |
