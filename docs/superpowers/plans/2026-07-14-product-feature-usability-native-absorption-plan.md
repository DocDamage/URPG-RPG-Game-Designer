...
Current PFU-I1 rerun (2026-07-16): both `creator journey` tests passed, `check_creator_journey.ps1` accepted the historical baseline report, `check_creator_vertical_slice.ps1` passed, and `ctest --preset dev-spatial --output-on-failure` passed all 3 tests after four stale assertions were corrected to the current toolbar, failed-restore, ordered-page, and rejected-replacement contracts. Additional focused evidence passed for gameplay recipes (31 assertions/3 cases), Menu (211/18), Menu focus (3/1), Dialogue/Narrative (12/1), localization (123/24), accessibility (184/33), playtest session (25/2), Grid Part (1035/109), plugin compatibility (83/11), and asset attachment/revision safety (238/11). The strict PFU-I1 wrapper is currently blocked, not deferred: it correctly rejects the dirty implementation worktree before qualification. A running editor also owns `SDL2d.dll`, so updated executables link and run but CMake's post-link runtime-DLL copy reports access denied; the process was not terminated because it may contain unsaved user state. Clean Debug/Release provenance, package/install smoke, and manual native-shell evidence therefore remain open and no PFU-I1 pass is claimed.
...
Bounded implementation evidence (2026-07-15): `AssetTransformRevisionService` creates immutable, SHA-256 source/provenance-linked atlas metadata, crop/nearest-neighbor-scale PNG, fixed-palette PNG, frequency-selected exact-RGBA palette PNG, individual-tile PNG directories, and PCM16 WAV trim/fade/gain revisions from validated runtime-ready promoted assets. It validates dimensions/frame bounds, stages output and manifests via same-directory rename, reuses an identical complete revision, rejects invalid/colliding plans, and supports verified cleanup of an un-attached manifest plus its validated deterministic PNG/WAV/tile siblings without modifying source media. Tileset slicing requires an exact margin/spacing grid and produces row-major zero-padded PNGs. Explicit palette mapping uses a creator-authored ordered two-to-256 RGBA palette with deterministic nearest-color/tie behavior; automatic extraction counts exact source RGBA colors, ranks frequency descending with RGBA tie breaking, and records its selected palette and requested bound before the same mapping. Either palette route may instead use a fixed-point, scan-ordered Floyd-Steinberg RGBA dither; that mode is part of revision identity and manifest and uses no floating-point rounding. Neither claims perceptual clustering. WAV manifests record derived loop bounds, 64 waveform peaks, sample peak/RMS dBFS metrics, and a versioned normalized PCM16 sample delta across an enabled loop boundary; the latter is a review signal, not LUFS conformance or a listening test. The native asset model also persists user favorites and named collections via existing user settings using opaque stable keys, without promoting or retaining raw external paths; the workspace has a bounded read-only comparison view. The Assets workspace now provides parameter-entry creation of atlas metadata, image crop/nearest-neighbor-scale, two-to-256 color fixed-palette, automatic palette extraction with optional fixed-point dithering, exact-grid tileset, and PCM16 trim/fade/gain revisions. A PCM16 source can now be read-only inspected for authoritative 128-bucket waveform data and a bounded 32-by-24 DFT-magnitude grid, then dragged into exact source-frame trim bounds before the same explicit revision action; the grid is a visual source preview, not persisted revision QA, loudness evidence, or a listening test. Valid audio outputs show the persisted peak/RMS and loop-boundary signal. A runtime-ready, license-backed promoted audio asset can also persist a validated locale, stable take ID, and optional runtime-ready muted-alternative audio asset ID in its governed manifest; the existing license ID remains rights authority, and this metadata flows through the existing library-to-attachment manifest path only when a creator explicitly attaches or replaces the project attachment. Single-output manifests feed directly to reviewed attachment; atlas metadata and tileset manifests are visible operation results until their native owners admit them. After preserving explicit direct-path compatibility, the native audio backend resolves a stable audio asset ID through only a matching runtime-ready project attachment manifest and canonical payload contained under the configured `content` root before its legacy filename probes, so attached audio can enter the existing preview/runtime core without raw path authority. Validated single-output crop/palette/PCM16 derived revisions can travel through an Assets-workspace review/confirm action to the existing receipt-backed project attachment owner as stable revision-qualified assets, preserving transform provenance and entering the normal contextual asset picker. Attachment staging writes a fail-closed sidecar reference ledger, so revision removal is blocked while a project attachment is prepared or live; a recovery action finalizes a prepared marker only from exactly one matching durable project manifest and reloads that project’s attachments, while metadata-only revisions remain rejected and tileset bundles use their dedicated assignment owner. The Assets workspace exposes a confirmation-gated action for the existing verified removal of a selected un-attached revision; it reconstructs the deterministic asset/revision path and cannot detach protected project content. The workspace can also explicitly remove only an asset’s hash-named unpublished `.png.tmp`, `.wav.tmp`, `.json.tmp`, and `.tiles.tmp` staging artifacts after preflighting every candidate type; final revisions, ledgers, and arbitrary temporary files remain untouched. This is cleanup, not job resumption or interrupted-publication recovery. A spectrogram editor, other audio codecs/loudness/caption work, direct Map placement/history, device/manual runtime/package evidence, cancellation, safe detach tooling, and recovery beyond durable-proof reconciliation remain PFU-03 scope. See `docs/superpowers/work-packets/PFU-03-atlas-metadata-revision-contract.md`.

Voice-metadata audit increment (2026-07-15): the existing read-only Project Localization Audit now examines the project-contained runtime-ready attached audio manifest behind every saved Dialogue voice reference. Alongside the existing voice/caption findings, it visibly reports the governed `voice_take` locale and take ID plus metadata and muted-alternative attachment status. It diagnoses absent metadata, malformed schema/locale/take ID, and a requested muted alternative that is not a different valid attached project audio asset. It performs no reconciliation, attachment rewrite, caption generation, locale fallback, playback qualification, or listening claim.
...
Localization audit increment (2026-07-15): a native, read-only project localization audit now compares valid locale-bundle keys with saved Dialogue Graph node/caption and Quest Objective Graph node references. It exposes missing keys, per-locale missing referenced keys, missing locale font-profile declarations, deterministic unused-key candidates, typed owner paths/IDs, and malformed-document diagnostics from the Map creator route without modifying authored data. It also lists persisted Dialogue node voice/caption pairs and diagnoses a missing project-contained runtime-ready attached audio asset, a voice without a caption key, a caption without a voice, or a missing caption key. For a valid attached voice it also surfaces governed `voice_take` locale/take metadata and diagnoses missing or malformed metadata plus missing/invalid requested muted-alternative attachments. The same route provides a pure pseudo-localization visual-expansion preview; it does not rewrite a bundle or qualify rights, locale fallback, playback, listening, RTL/IME/glyph/plural/layout support. Unused candidates are not deletion authority. See `docs/superpowers/work-packets/PFU-06-project-localization-reference-audit.md`.
