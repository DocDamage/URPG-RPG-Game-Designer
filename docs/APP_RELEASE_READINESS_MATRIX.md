...
Latest audit-remediation checkpoint: Phase 0 through Phase 12 plus the 2026-05-05 public-release record pass landed for the bounded `v0.1.0` package scope. The annotated `v0.1.0` tag exists and targets commit `1d3debb95b6df1d09996e723cc616369cfca99c6`. Current `development` is post-tag, so any follow-up public release or broader completion claim requires fresh gate evidence on the new target commit. The mandatory completion-scope lock remains active for current-codebase claims. Current-codebase scan status on 2026-05-27: release-required app assets remain governed by `check_release_required_assets.ps1`; this checkout is not LFS-free, but `check_lfs_release_scope.ps1` reconciles all 307,388 LFS-tracked paths as either deferred curated-library payloads or governance evidence, with zero release-required LFS paths and zero unknown LFS paths; generated/local retired-root tracking has been removed from the Git index (`check_no_generated_tracked_files.ps1` passes). Security/lifetime hardening from `docs/superpowers/plans/2026-05-26-finish-current-codebase.md` is implemented for the focused lanes, with sanitizer and cross-platform picker verification still requiring appropriate external runners/toolchains.
...
| Release-required asset hydration and LFS scope | `RECONCILED_FOR_BOUNDED_SCOPE` | `.gitattributes`; `resources/icons/*.png`; `imports/manifests/asset_bundles/BND-001.json`; `imports/manifests/asset_bundles/BND-003.json`; `tools/ci/check_release_required_assets.ps1`; `tools/ci/check_lfs_release_scope.ps1`; `docs/asset_intake/LFS_SCOPE_RECONCILIATION.md`; `imports/reports/asset_intake/lfs_release_scope_report.json` | `./tools/ci/check_release_required_assets.ps1`; `./tools/ci/check_lfs_release_scope.ps1` | P5-005; P6-002; P9-001 | Release-required app assets are governed by the asset gate and current LFS scope is reconciled: 307,388 LFS-tracked paths are classified as deferred curated-library payloads or governance evidence, with zero release-required LFS paths and zero unknown LFS paths. Raw/source asset packs remain outside the release package path. Broader asset claims require project-selected hydration/access evidence. |
...
| --- | --- | --- |
| Repository-wide source/vendor LFS budget/access | `RECONCILED_FOR_BOUNDED_SCOPE` | Current checkout reports 307,388 LFS-tracked paths, all classified by `check_lfs_release_scope.ps1`. Do not depend on broad source/vendor or promoted asset payloads for release claims unless hydrated and project-selected; keep broad raw/source intake local and ignored unless selected assets are promoted through governed manifests. |
| Repository generated/local tracking | `VERIFIED` | `git ls-files build-local Testing/Temporary third_party "itch/loose"` returns no tracked paths after `git rm --cached`; `.\tools\ci\check_no_generated_tracked_files.ps1` passes. |
