...
| Release-required asset hydration | `PARTIAL` | `.gitattributes`; `resources/icons/*.png`; `imports/manifests/asset_bundles/BND-001.json`; `imports/manifests/asset_bundles/BND-003.json`; `tools/ci/check_release_required_assets.ps1`; current `git lfs ls-files --name-only` scan | `./tools/ci/check_release_required_assets.ps1`; `git lfs ls-files --name-only` | P5-005; P6-002; P9-001 | Release-required app assets are still governed by the asset gate, but the current checkout is no longer accurately described as zero-LFS: 32,229 LFS-tracked normalized asset paths were present on 2026-05-26. Raw/source asset packs remain outside the release package path. Broader asset claims require LFS hydration/access evidence or removal from tracked release-facing scope. |
...
| --- | --- | --- |
| Repository-wide source/vendor LFS budget/access | `OPEN` | Current checkout reports 32,229 LFS-tracked normalized asset paths, so older zero-LFS wording is stale. Do not depend on broad source/vendor or promoted asset payloads for release claims unless hydrated and verified; keep broad raw/source intake local and ignored unless selected assets are promoted through governed manifests. |
