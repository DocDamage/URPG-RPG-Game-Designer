...
- Current checkout drift as of 2026-05-26: root `third_party/` and root `itch/loose/` are removed from the Git index by the current cleanup change, and `git lfs ls-files --name-only` reports 32,229 LFS-tracked normalized asset paths. Treat broad LFS hydration expectations as release-scope constraints until reconciled for a target release.
