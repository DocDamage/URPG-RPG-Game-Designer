...
| 11 | `SRC-013` | Local itch loose asset drop | `local://itch/loose` | `—` | `2026-05-03` | `direct_asset_pack` | sprites, tilesets, UI, VFX, props | Editor asset browser discovery from local itch loose payloads; template polish candidates after project selection; release-eligible app/library selection with deferred default export bundling | `mirrored` | `direct_ingest_when_captured` | `cc0_public_domain_promoted` | `promoted` |
| 12 | `SRC-014` | Local 2026-05-04 asset drop with ModernUI | `local://asset-drop/2026-05-04` | `—` | `2026-05-04` | `direct_asset_pack` | UI, characters, portraits, sprites, archives, source assets | ModernUI unsliced UI source sheets; ModernUI portrait-generator parts; extracted CuteSCKR/Human pack discovery; curated future promotion after project selection | `mirrored` | `direct_ingest_when_captured` | `cc0_public_domain_promoted` | `promoted` |
| 13 | `SRC-015` | Local CuteSCKR tileset drop | `local://ingest stuff 5-4-26/CuteSCKR` | `—` | `2026-05-05` | `direct_asset_pack` | tilesets, rpg_maker, map, environment | Level Builder gameplay tile palette, Mario Maker-style map authoring, and template-scoped starter/full-library catalogs | `partial_extract` | `bounded_editor_ready_slice_after_local_extract` | `cc0_or_public_domain_per_source_license` | `partially_promoted` |
...
- SRC-011 local AssetsForMyGame catalog: `imports/reports/asset_intake/assets_for_my_game_promotion_catalog.json` plus capped category shard parts under `imports/reports/asset_intake/assets_for_my_game_promotion_catalog/` record 12,985 local-use records after extracting 58 ZIP/RAR archives with zero failures. Raw binaries remain local under `imports/raw/assets_for_my_game`; export remains blocked pending per-pack attribution and curated bundle promotion.
- SRC-012 local GoDoGenUI catalog: `imports/reports/asset_intake/godogenui_assets_promotion_catalog.json` plus capped category shard parts under `imports/reports/asset_intake/godogenui_assets_promotion_catalog/` record 29,516 supported local-use records from 46,285 mirrored files. Unsupported source/tool/development files are reported but not exposed as usable editor assets. `BND-005` promotes a bounded 6-file CC0/public-domain tiles/VFX subset under `imports/normalized/src012_cc0_tiles_vfx/`; all remaining raw binaries stay local under `imports/raw/godogenui_assets` and export-blocked pending per-pack attribution plus curated bundle promotion.
- SRC-014 local 2026-05-04 catalog: `imports/reports/asset_intake/ingest_20260504_promotion_catalog.json` plus category shards under `imports/reports/asset_intake/ingest_20260504_promotion_catalog/` record 1,076 supported local-use records from `ingest stuff 5-4-26/`. `imports/reports/asset_intake/src014_archive_extraction_report.json` records extraction of all 93 CuteSCKR/Human ZIP/RAR archives into ignored raw quarantine under `imports/raw/src014_20260504_extracted/`, and `imports/reports/asset_intake/ingest_20260504_extracted_promotion_catalog.json` plus shards record 1,397 supported extracted local-use assets. `BND-009` catalogs the ModernUI unsliced UI source sheets under `imports/normalized/src014_modern_ui/` with deferred distribution; `BND-010` promotes 953 ModernUI portrait-generator PNG/Aseprite assets under `imports/normalized/src014_modernui_portrait_generator/` with deferred distribution.
- SRC-015 CuteSCKR game-maker catalog: `imports/manifests/asset_sources/SRC-015.json` and `imports/manifests/asset_bundles/BND-011.json` record the bounded CuteSCKR Level Builder tileset lane. The complete generated slices are exposed through `content/part_catalogs/generated/cutesckr_all_parts.json` and the opt-in aggregate `content/part_catalogs/game_maker_all_parts.json`, while default startup remains on `content/part_catalogs/base_jrpg_parts.json`.
- UI theme validation manifests: `content/ui_themes/complete_ui_essential_flat.json` and `content/ui_themes/wenrexa_hologram.json` record game UI theme candidates from local packs. Completeness reports live under `imports/reports/ui_theme_validation/`; only Complete UI Essential currently passes `gameUiReady`.
...
- Phase 9 release starter skin lane: `imports/manifests/asset_bundles/BND-003.json` provides the release-required repo-generated UI frame/chrome, cohesive skin metadata, and battle VFX proof from `SRC-009`.
...
- SRC-014 ModernUI lane: `imports/manifests/asset_bundles/BND-009.json` catalogs unsliced ModernUI source sheets/animations with attribution record `imports/reports/asset_intake/attribution/BND-009_src014_modern_ui.json`; assets remain deferred until slicing metadata makes them runtime-ready.
- SRC-014 ModernUI portrait-generator lane: `imports/manifests/asset_bundles/BND-010.json` promotes 953 CC0 PNG/Aseprite portrait part/source files with attribution record `imports/reports/asset_intake/attribution/BND-010_src014_modernui_portrait_generator.json`; all assets are release-eligible but `distribution: "deferred"` until a project explicitly selects them.
...
| 2026-05-05 | Added `SRC-014` for the `ingest stuff 5-4-26` local drop, catalog-normalized all supported assets, and cataloged `BND-009` as deferred CC0 ModernUI unsliced UI source sheets under `imports/normalized/src014_modern_ui/`. |
| 2026-05-05 | Extracted all 93 `SRC-014` CuteSCKR/Human ZIP/RAR archives into ignored raw quarantine, catalog-normalized 1,397 extracted image assets, and promoted 953 ModernUI portrait-generator assets through deferred `BND-010`. |
| 2026-05-05 | Added `SRC-015` and `BND-011` for the bounded CuteSCKR Level Builder/game-maker tileset lane, generated the opt-in `game_maker_all_parts` catalog, and recorded UI theme completeness manifests for Complete UI Essential and Wenrexa. |
