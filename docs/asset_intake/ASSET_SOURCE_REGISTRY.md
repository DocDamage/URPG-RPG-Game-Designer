...
- Current checkout drift as of 2026-05-26: root `third_party/` and root `itch/loose/` still have tracked files, and `git lfs ls-files --name-only` reports 32,229 LFS-tracked normalized asset paths. Treat those as repository hygiene and release-scope blockers until the tracked roots are removed/reclassified and LFS hydration expectations are reconciled.
