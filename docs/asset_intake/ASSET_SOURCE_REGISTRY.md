...
- Current checkout state as of 2026-05-27: root `third_party/` and root `itch/loose/` are removed from the Git index by the current cleanup change, and `.\tools\ci\check_lfs_release_scope.ps1` reconciles 307,388 LFS-tracked paths as deferred curated-library payloads or governance evidence with zero release-required LFS paths. Treat broad LFS hydration expectations as project-selection constraints before broader release claims.
