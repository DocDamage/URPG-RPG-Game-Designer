...
- Phase 9 release starter skin lane: `imports/manifests/asset_bundles/BND-003.json` retains repo-generated UI frame/chrome fallback rows and the release-required battle VFX proof from `SRC-009`; ModernUI now covers the required app UI skin.
...
- SRC-014 ModernUI lane: `imports/manifests/asset_bundles/BND-009.json` promotes the app-wide ModernUI skin subset with attribution record `imports/reports/asset_intake/attribution/BND-009_src014_modern_ui.json` and install/export payloads under `content/ui/modern/`.
...
| 2026-05-05 | Added `SRC-014` for the `ingest stuff 5-4-26` local drop, catalog-normalized all supported assets, and promoted `BND-009` as the CC0 ModernUI app skin under `content/ui/modern/` plus `imports/normalized/src014_modern_ui/`. |
