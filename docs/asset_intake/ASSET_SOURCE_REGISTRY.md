...
| 11 | `SRC-013` | Local itch loose asset drop | `local://itch/loose` | `—` | `2026-05-03` | `direct_asset_pack` | sprites, tilesets, UI, VFX, props | Editor asset browser discovery from local itch loose payloads; template polish candidates after project selection | `mirrored` | `direct_ingest_when_captured` | `cc0_public_domain_promoted` | `promoted` |
| 12 | `SRC-014` | Present local raw asset payloads | `local://imports/raw` | `—` | `2026-05-26` | `direct_asset_pack` | audio, character, data, font, map, props, tilesets, UI, VFX | Release-eligible asset library payloads from currently present raw payloads; project-selected export payloads after template or project selection | `mirrored` | `direct_ingest_when_captured` | `project_provided_cc0_public_domain_with_fallback` | `promoted` |
| 13 | `SRC-015` | Sibling folder bulk assets | `local://C:/Users/dferr/dev` | `—` | `2026-05-26` | `direct_asset_pack` | audio, character, font, map, props, tilesets, UI, VFX | Release-eligible asset library payloads discovered one directory below the URPG workspace parent; project-selected export payloads after template or project selection | `mirrored` | `direct_ingest_when_captured` | `project_provided_cc0_public_domain_with_fallback` | `promoted` |
...
| P2 | Ambient/background audio | Discovery-index audio candidates | Prefer reviewed, style-coherent selections with clear attribution metadata | Discovery backlog only |
...
- Current recorded state: 15 cataloged sources, 9 normalized/promoted proof or bulk lanes, 6 local catalog-normalized raw quarantine sources, 4 cataloged implementation candidates
...
- Third-party/itch local index: `imports/reports/asset_intake/third_party_itch_ingest_summary.json` records the asset DB ingest over local raw quarantine roots including `imports/raw/third_party_assets/itch-assets`, source-only RPG Maker/Aseprite/root-drop archives, `imports/raw/third_party_assets/huggingface`, `imports/raw/third_party_assets/external-repos`, `imports/raw/third_party_assets/github_assets`, and `imports/raw/itch_assets/loose`. `BND-010` promotes currently present app-usable raw payloads from those roots into `imports/normalized/present_raw_local_bulk/`; rows are `distribution: "deferred"`, `release_eligible: true`, and `license_cleared: true` under `docs/compliance/asset_licenses/local-bulk-assets-CC0.md`.
- Sibling-folder bulk intake: `BND-011` promotes app-usable media/map/font/VFX payloads found one directory below `C:\Users\dferr\dev` into `imports/normalized/sibling_bulk_assets/`; rows are `distribution: "deferred"`, `release_eligible: true`, and `license_cleared: true` under `docs/compliance/asset_licenses/local-bulk-assets-CC0.md`.
- Current checkout state as of 2026-05-27: `.\tools\ci\check_lfs_release_scope.ps1` reconciles 307,388 LFS-tracked paths as governed promoted-library payloads or governance evidence with zero release-required LFS paths, and `.\tools\ci\check_promoted_asset_library.ps1` validates the promoted manifest inventory. Treat broad LFS hydration expectations as project-selection constraints before broader release claims.
...
- First audio proof lane: `imports/manifests/asset_bundles/BND-002.json`; the current release uses a silent audio policy, and newly promoted audio must use OGG output from `tools/assets/convert_audio_to_ogg.py`
...
- Present raw local bulk lane: `imports/manifests/asset_bundles/BND-010.json` records 30,628 source rows, 14,922 canonical normalized payload files, and 15,706 duplicate source rows sharing canonical payloads; attribution record is `imports/reports/asset_intake/attribution/BND-010_present_raw_local_bulk.json`.
- Sibling folder bulk lane: `imports/manifests/asset_bundles/BND-011.json` records 339,499 source rows, 277,849 canonical normalized payload files, and 61,650 duplicate source rows sharing canonical payloads; attribution record is `imports/reports/asset_intake/attribution/BND-011_sibling_bulk_assets.json`.
...
| 2026-05-26 | Added `SRC-014` and `BND-010` to promote all currently present app-usable raw payloads into a release-eligible deferred normalized lane under project-provided CC0/fallback license evidence. |
| 2026-05-26 | Added `SRC-015` and `BND-011` to promote app-usable sibling-folder media/map/font/VFX payloads into a release-eligible deferred normalized lane under the same license evidence. |
