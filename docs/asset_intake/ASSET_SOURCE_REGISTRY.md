...
- Third-party/itch local index: `imports/reports/asset_intake/third_party_itch_ingest_summary.json` records the asset DB ingest over `third_party/itch-assets`, `third_party/rpgmaker-mz`, `third_party/huggingface`, `third_party/aseprite`, `third_party/external-repos`, `third_party/github_assets`, and `itch/loose`. This is a local catalog/index pass, not release promotion.
