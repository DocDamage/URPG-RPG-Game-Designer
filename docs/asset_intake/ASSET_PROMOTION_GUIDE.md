...
- ExportPackager rejects raw intake roots, external-store roots, source-only files, unresolved Git LFS pointers, and discovered assets without explicit attribution. Use promoted bundle manifests or project-local runtime asset roots with `asset_licenses.json` evidence instead of broad source discovery.
