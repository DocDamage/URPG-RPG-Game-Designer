...
- [ ] Spatial Presentation: `SpatialMapOverlay` headers and `test_spatial_editor.cpp` coverage exist, but `editor/spatial/*` panels are not yet registered as compiled sources in the build graph. Core presentation runtime (`presentation_runtime.cpp`, `release_validation.cpp`) is registered.
- [ ] Editor Tooling: `ElevationBrushPanel` and `PropPlacementPanel` headers exist and are exercised by `test_spatial_editor.cpp`, but no `.cpp` implementation is registered in the build graph.
