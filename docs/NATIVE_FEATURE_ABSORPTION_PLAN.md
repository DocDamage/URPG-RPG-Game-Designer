...
| Message flow, text layout, and dialogue chrome | Current evidence is primarily API-surface parity rather than routed lifecycle coverage: compat burn-down includes `drawTextEx`, `textWidth`, `textSize`, and face rendering parity, but a routed dialogue anchor is still missing | `Window_Base` text/face compat surfaces plus future message-window import mappings | Message / Text Core | Dialogue inspector, rich-text preview, face/namebox layout preview, localization-aware validation | Native message runner plus text layout engine | Import should translate escape codes, timing rules, portraits/faces, and message-box variants into native dialogue schema with a compat fallback only for unsupported control flows | `tests/unit/test_window_compat.cpp` text and face coverage; blueprint message/text contracts |
| Save slot metadata, autosave routing, and recovery UX | Save-data reload anchor verifies slot metadata and autosave flows stay aligned with routed menu/dashboard entry points across plugin reload, backed by DataManager save/header-extension contracts | `DataManager` compat save/header APIs plus routed save-data fixture wiring | Save / Data Core | Save-slot inspector, save metadata panel, recovery diagnostics view, template save-policy settings | Native save catalog, metadata index, and recovery orchestrator | Imported save plugins should map slot presentation, header-extension metadata, autosave affordances, and recovery flags into native save schema rather than opaque plugin config | `tests/compat/test_compat_plugin_fixtures.cpp` save-data reload anchor, `tests/unit/test_data_manager.cpp`, and save runtime/recovery tests |
...
- Message / Text Core still needs a routed dialogue/message anchor before its first-wave spec is fully evidence-backed.
- Save / Data Core now has a first routed anchor, but it still needs broader failure-path and import-shape evidence before the full spec set is complete.
