...
- The active implementation plan is [`docs/superpowers/plans/2026-07-14-product-feature-usability-native-absorption-plan.md`](superpowers/plans/2026-07-14-product-feature-usability-native-absorption-plan.md). Its bounded M0-M5 foundation is implemented through commit `d607eecc`; checked task boxes record implemented contracts, not manual or release signoff.
...
- LFS wording remains bounded and now reconciled: `.\tools\ci\check_lfs_release_scope.ps1` reports 307,388 LFS-tracked paths in this checkout, with zero release-required LFS paths and zero unknown paths outside the governed allowlist. Release-required app assets are verified separately by `check_release_required_assets.ps1`, and the full promoted manifest inventory is validated by `check_promoted_asset_library.ps1`. The branch must not be described as zero-LFS; shipped-game claims require exact project-selected payloads.
...
2026-05-27 release protection and high-end MZ compatibility evidence checkpoint:
- Release bundle/script protection is implemented for the in-tree scope: `obfuscateScripts=true` now emits a deterministic `urpg_script_minify_v1` script payload plus `runtime/script_transform_manifest.json`, and generated bundles now declare `authenticated_release_bundle_v1`, `hmac_sha256_bundle_v2`, and `manifest_payload_target_v2` metadata while retaining runtime tamper/replay validation. This is authenticated engine bundle protection, not external public-key signing, platform notarization, or store publication.
- The high-end MZ lane now has repo-owned reference capture descriptors, an expanded legal two-map event corpus descriptor, a backend parity evidence aggregator, and workbench parity counts. These records intentionally keep `release_authoritative=false`; they are evidence plumbing and backend comparison data, not a claim of proprietary RPG Maker MZ runtime parity.
- Cloud sync, live services, and store publishing are future-update scope for this pass. In-tree cloud remains local/hidden unless a reviewed remote provider is supplied, and proprietary store credentials/publication remain project or release-owner configuration boundaries.
- The signed-off `compat_bridge_exit` scope remains READY only for the bounded import, validation, migration, diagnostics, and verification bridge. A separate high-end MZ compatibility expansion lane is now in progress and explicitly non-authoritative for full RPG Maker MZ runtime parity.
- New deterministic evidence surfaces cover legal full-project corpus descriptors, project-level compatibility scoring, event command coverage analytics, plugin repair estimates and migration suggestions, a visual-diff report contract, an opt-in runtime parity mode marked experimental, backend comparison evidence, and a creator migration workbench snapshot.
- Runtime parity remains experimental until release-owner review accepts the evidence set. The current reference captures are legal descriptors and hashes, not bundled proprietary runtime captures or a captured-reference parity claim.

2026-05-26 Perspective 2D depth-pass checkpoint:
- `SpatialAuthoringWorkspace` now has a durable P2D map-authoring slice: `tiles` toolbar mode, promoted asset tile palette option rows, palette search plus tileset/category filters, tile thumbnail/category preview metadata, selected palette option and tileset/tile state, brush-size painting and erasing, RPG Maker-style tileset page metadata for pages A-Z, autotile metadata, animated tile frames, passage/collision flags, terrain tags, region IDs, priority/star passability, coordinate tile previews, named visible/locked layer rows, single-layer and multi-layer selection, bulk layer visible/locked toggles, layer duplicate/clear/delete operations, duplicate/delete selected-layer operations, canvas tile painting, event/object placement rows, event movement, editable/removable event command rows, RPG Maker-style event page rows, page reorder/duplicate/delete operations, editable/removable page conditions and page commands, variable/switch comparison condition rules, conditional branch command rows with nested true/false child commands, active-page preview state, event execution preview traces, live runtime execution for `show_text`, `transfer_player`, `change_switch`, `change_variable`, `change_self_switch`, `change_gold`, `change_item`, `move_route`, `call_common_event`, and `conditional_branch`, persistent runtime state snapshots for switches/variables/self-switches/inventory/gold/player position, project/database references for actors/items/switches/variables/common events/maps/starting party/transfers/encounters/assets/save-load profiles, RPG-editor UI model snapshots for page tabs, condition editor, command list, branch tree, command picker, palette/layer panels, playtest controls, and export controls, playtest execution handoff trace bundles, JSON draft save/load results, runtime playtest manifest results, deterministic package inventory/signature proof, export package manifest results, creator UX readiness summaries, release asset/LFS policy gate results, playtest readiness diagnostics, and export readiness results.
...
- Phase 4 promotes `character_identity` to `READY` for the claimed character identity and appearance-part authoring scope: creator-supplied portrait/field/battle import rows, governed promotion paths, source metadata, dimensions, attribution diagnostics, part-library accept/reject/archive/assign snapshots, persisted assignments, runtime preview composition, importer category mapping, and closure signoff are landed. Final-quality portrait art and cohesive character art direction are resolved for the bounded release by `docs/asset_intake/FINAL_ART_CONTENT_SCOPE.md`: future project selections need exact asset, hydration, attribution, and art-direction evidence before broader final-art claims.
- Phase 5 promotes `accessibility_auditor` and `audio_mix_presets` to `READY` for the claimed cross-cutting scope: release top-level panel renderer-derived contrast coverage, existing live accessibility adapters, deterministic audio backend/device matrix fixtures, muted release fallback diagnostics, and `AudioMixPanel` matrix exposure are landed. JRPG, turn-based RPG, and visual-novel templates now show accessibility/audio bars as `READY` while retaining their remaining performance/input/template-governance blockers.
- Phase 7 promotes `achievement_registry`, `mod_registry`, and `analytics_dispatcher` to `READY` for the claimed external-service boundary scope: shared provider-profile statuses, missing-credential and unsupported-provider diagnostics, reviewed-production gating, and editor snapshot diagnostics are landed. Proprietary store SDK credentials, external marketplace operation, payments/reviews, and qualified public privacy/legal approval remain project-configuration or release-owner review boundaries rather than unfinished engine code.
...
- Phase 4 persistence/input work is verified: release authoring save/load paths, runtime menu input forwarding, pause/resume input suppression, controller defaults, and touch remap diagnostics have focused tests.
- Phase 0 build blockers are closed for the editor release registry: release top-level navigation now contains only app-shell-wired panels, `developer_debug_overlay` is dev-only, and the compat save-data lifecycle panel test no longer mixes stale execution diagnostics into failure diagnostics.
- Phase 1 navigation exposure work is complete through P1-003: `level_builder` is wired into the editor app shell with the real native `LevelBuilderWorkspace`, every release top-level id has an app factory, every compiled editor panel/workspace has one registry exposure owner, and registry owner paths are executable-test verified.
- Phase 2 truthfulness work is complete through P2-002: release top-level editor owners are scanned for ungoverned placeholder/mock/stub/dev-bootstrap terms, misleading placeholder wording was removed from runtime/package comments and logs, and truth reconciliation now fails if `READY` or `VERIFIED` claims cite fixture-only, mock-only, placeholder-backed, or dev-bootstrap evidence.
- Canonical docs updated in this checkpoint include `README.md`, `docs/APP_RELEASE_READINESS_MATRIX.md`, `docs/release/EDITOR_CONTROL_INVENTORY.md`, `docs/release/RELEASE_PACKAGING.md`, `docs/packaging.md`, `docs/agent/QUALITY_GATES.md`, and `docs/superpowers/plans/2026-04-30-release-surface-audit-execution-plan.md`.
- Verification performed during this checkpoint included editor registry/app-shell CTest lanes, `urpg_editor --headless --open-panel level_builder`, grid-part CTest lanes, compat lifecycle tests, release-required asset checks, install smoke, package smoke, PR tests, and the full local gate.

2026-04-30 native Level Builder checkpoint:
- `level_builder` is the release top-level native grid-part map editor surface; `spatial_authoring` is promoted to the release top-level Perspective 2D map editor surface.
...
- Export/release UX now projects platform checklist rows, missing asset/artifact report data, packaging diagnostics, signing/notarization profile status, and staged smoke evidence through the export preview result and editor panel snapshot while enforcing release-profile artifact policy before exact-ship claims.
...
- Export validator is `READY` for the claimed Phase 6 scope: runtime rejection of tampered bundles, temp-file-plus-atomic-rename publication, raw/source/external-store rejection diagnostics, missing attribution/LFS pointer enforcement, and release-profile signing/notarization/artifact-policy checks are landed. Production signing certificates, notarization accounts, and platform-store publication remain project configuration rather than repository-owned engine code.
...
- Recent execution slices landed:
  ...
  - `runtimes/compat_js/window_compat.*` and `runtimes/compat_js/window_sprite_compat.cpp` now give `Sprite_Character` and `Sprite_Actor` deterministic compat bitmap-handle ownership, reload, destructor cleanup, source-rect mapping, actor-motion lifecycle semantics, and QuickJS bindings that mutate/observe a bound deterministic sprite instance instead of silently no-oping as public stubs.
...
- [x] Mandatory: treat every remaining roadmap lane as dual-delivery work: runtime capability plus visual authoring/live preview.
...
- [x] Mandatory: keep heavy ML/research dependencies isolated under `tools/` or separate helper environments.
