...
- LFS wording remains bounded and now reconciled: `.\tools\ci\check_lfs_release_scope.ps1` reports 307,388 LFS-tracked paths in this checkout, with zero release-required LFS paths and zero unknown paths outside the governed allowlist. Release-required app assets are verified separately by `check_lfs_release_scope.ps1` and `check_release_required_assets.ps1`, but the branch must not be described as zero-LFS or final-art/content-complete based on deferred library inventory.
