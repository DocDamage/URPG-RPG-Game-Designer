...
- `level_builder` is the release top-level native map editor surface; `spatial_authoring` is nested/supporting tooling.
...
Phase 11 offline tooling closure is complete for the current 100-percent program scope. FAISS retrieval chunk manifests and retrieval bundles, SAM/SAM2-compatible segmentation manifests, and Demucs/Encodec-compatible audio manifests are artifact-producing offline jobs under `tools/`, with shared job-runner coverage proving the retrieval, vision, and audio entrypoints remain addressable without runtime ML dependencies. Phase 12 curated content/release-tag work remains mandatory before a final 100-percent release claim.
