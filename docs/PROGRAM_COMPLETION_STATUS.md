...
- Advanced template/product lanes are implemented for the starter-template scope: `tactics_rpg`, `arpg`, `monster_collector_rpg`, `cozy_life_rpg`, `metroidvania_lite`, `2_5d_rpg`, `roguelite_dungeon`, `survival_horror_rpg`, `farming_adventure_rpg`, `card_battler_rpg`, `platformer_rpg`, `gacha_hero_rpg`, `mystery_detective_rpg`, and `world_exploration_rpg`, plus 20 additional genre templates from `space_opera_rpg` through `faction_politics_rpg`, now have `TemplateRuntimeProfile` definitions, generated starter project support, starter manifests under `content/templates/`, template certification loops, cross-cutting bars, WYSIWYG showcase bindings, and aligned canonical specs/readiness rows.
