...
- Recent execution slices landed:
  ...
  - TD-10 is now explicitly closed as a strategy lane: the in-tree AI/cloud surface is intentionally bounded to `IChatService` plus deterministic `MockChatService` and `ICloudService` plus `LocalInMemoryCloudService`. Any live transport/provider integration is mandatory current backlog only if product scope reopens it, not a hidden claim in the current tree.
...
- [x] AI Copilot Native Infrastructure (Wave 2 Advanced):
  ...
  - [x] `IChatService` abstraction plus deterministic `MockChatService`; live provider integrations remain out-of-tree.
