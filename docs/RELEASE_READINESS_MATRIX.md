...
| `compat_bridge_exit` | `PARTIAL` | Yes | N/A | Yes | Yes | Yes | Yes | Compat bridge exit has truthful Phase 2 closure evidence, weekly regression cadence, diagnostics/export parity, and an explicit signoff artifact for the bounded import and verification lane. | Compat bridge exit signoff exists; promotion to `READY` requires human review of residual gaps. Curated corpus maintenance and ongoing truth upkeep remain mandatory current backlog. |
| `presentation_runtime` | `PARTIAL` | Yes | Yes | Partial | Yes | Yes | Yes | Presentation runtime, bridge, compiled spatial editor panels, end-to-end authoring→runtime proof path, and release validation harness are landed. | Real renderer backend integration, profile arena hot-reload, and broader polish governance remain mandatory current backlog. |
| `gameplay_ability_framework` | `PARTIAL` | Yes | Yes | Partial | Yes | Yes | Yes | Gameplay ability runtime, inspector/editor surfaces, structured diagnostics snapshot, end-to-end release-grade test, and closure signoff artifact are landed. | Schema contracts, compat-to-native migration mapping, and real gameplay loop integration remain mandatory current backlog. Promotion to `READY` requires human review of residual gaps. |
| `governance_foundation` | `PARTIAL` | No | No | Partial | No | Yes | Yes | Breaking-change detection, enhanced readiness checks, template specs, bidirectional readiness/truth drift detection, richer project-audit artifact checks across input/accessibility/audio/character/mod/analytics/performance, a bounded controller-binding runtime/panel plus input governance script/fixture contract, a canonical release-signoff workflow artifact, and structured signoff contracts for the current human-review-gated subsystem lanes are now landed. | Full release-signoff enforcement beyond the current artifact-backed audit, workflow, and drift/date/status checks remains mandatory current backlog. |
| `character_identity` | `PARTIAL` | Yes | Yes | Yes | Yes | Yes | Yes | Character identity runtime, editor creator panel, bounded validation runtime, ECS component/system integration, and deterministic spawner are landed with schema and focused tests. | Character identity validator/governance coverage is landed; full Create-a-Character workflow (runtime character creation UI and appearance preview pipeline) remains mandatory current backlog. |
...
| `accessibility_auditor` | `PARTIAL` | Yes | Yes | Yes | Yes | Yes | Yes | Accessibility auditor with missing-label, focus-order, contrast, and navigation rules is landed. Menu inspector live-ingestion adapter and `check_accessibility_governance.ps1` CI gate are landed. | Menu adapter is the first bounded editor surface; spatial, audio, and battle adapters remain mandatory current backlog. Full surface coverage beyond menu requires additional dedicated adapters.
| `visual_regression_harness` | `PARTIAL` | Yes | No | No | No | Yes | Yes | Visual regression harness with golden file management, diff heatmaps, approval tooling, a file-backed executable harness gate, and one bounded OpenGL-enabled renderer-backed capture lane with committed clear-frame, full-frame-rect, and inset-rect goldens enforced in local gates and Gate 1 CI is landed. | Broader scene/backend coverage remains mandatory current backlog. |
...
| `export_validator` | `PARTIAL` | Yes | Yes | Yes | Yes | Yes | Yes | Platform-specific export validator with per-target requirement checking, fixture-backed validation coverage, a now-explicit split between packager preflight readiness and post-export artifact validation, diagnostics coverage for divergent preflight/post-export states, PowerShell CI script with JSON output, one bounded deterministic `data.pck` manifest bundle, bounded real Windows/Web/Linux/macOS bootstrap paths, bundle-level lightweight RLE-plus-XOR payload protection plus keyed integrity-tag validation, bounded repo-owned content bundling for readiness/schema/level-library roots, and governed manifest-driven promoted-asset staging support is landed. | Automatic project-wide asset discovery/bundling, full native packaging/signing/notarization, and stronger cryptographic signing/runtime enforcement remain mandatory current backlog. |
