...
| `export_validator` | `PARTIAL` | Yes | Yes | Yes | Yes | Yes | Yes | Platform-specific export validator with per-target requirement checking, fixture-backed validation coverage, PowerShell CI script with JSON output, diagnostics panel with post-export artifact surfacing, and one bounded real Windows launch smoke path is landed. | Real asset bundling remains stubbed, non-Windows executable synthesis is still synthetic, and security hardening beyond the current smoke lane remains future work. |
| `mod_registry` | `PARTIAL` | Yes | Yes | Yes | Yes | Yes | Yes | Mod registry with manifest registration, dependency resolution, load-order topology, bounded validation runtime, and editor manager panel is landed. | Mod validator/governance coverage is landed; live mod loading, sandboxed script execution, and mod-store integration remain future work. |
