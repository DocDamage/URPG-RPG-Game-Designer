...
| `governance_foundation` | `PARTIAL` | No | No | Partial | No | Yes | Yes | Breaking-change detection, enhanced readiness checks, template specs, bidirectional readiness/truth drift detection, and richer project-audit artifact checks are now landed. | Full release-signoff enforcement beyond the current artifact-backed audit and drift/date/status checks remains future work. |
