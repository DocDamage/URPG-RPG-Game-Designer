...
| `analytics_dispatcher` | `PARTIAL` | Yes | Yes | Yes | Yes | Yes | Yes | Opt-in analytics dispatcher with deterministic tick counters, event buffering, circular drop, local and remote telemetry upload, editor panel, and bounded validator/governance coverage is landed. | Analytics validator/governance coverage, telemetry upload, and session aggregation are landed; production privacy review workflow remains mandatory current backlog. |
...
| Current checkout LFS footprint | `PARTIAL` | P5-005 removed current tracking for source-drop files and source-only `fantasy-platformer-game-ui/PSD/` files. The 2026-04-30 baggage passes also remove current Git tracking for `imports/raw/more_assets/`, `imports/raw/itch_assets/loose/`, broad `imports/raw/third_party_assets/itch-assets/`, source-only RPG Maker/Aseprite intake, and `imports/root-drop/archives/` while preserving local working files as ignored raw quarantine. Curated RPG Maker plugin drop-ins, plugin release reports, and Hugging Face fixtures remain tracked because gates/tests reference them. | Continue reducing required LFS to release-owned/promoted assets only; history still contains prior LFS payload and cannot be considered hydrated until remote access is restored. |
