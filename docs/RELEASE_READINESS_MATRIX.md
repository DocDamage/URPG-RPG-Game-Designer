...
| `export_validator` | `PARTIAL` | Yes | No | Yes | No | Yes | Yes | Platform-specific export validator with per-target requirement checking, packager preflight enforcement, diagnostics parity, and PowerShell CI script is landed. | Integration with a real export artifact pipeline and a broader automated CI export lane remain future work. |
