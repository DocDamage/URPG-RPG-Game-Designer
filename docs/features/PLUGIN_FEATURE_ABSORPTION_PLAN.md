...
| `features needing to be integrated.txt` / Fast Travel | Visual waypoint menu with preview, unlock conditions, fallback common events | World exploration template already claims fast travel conceptually | Needs native `world/fast_travel` runtime/editor implementation. |
...
| `VehicleEffects.js` | Vehicle BGM/BGS, speed, encounters, interiors, transitions | `mount_vehicle_system_fixture`, audio resolver, map transfer systems | Extend native vehicle profiles and editor panel. |
...
3. Presentation media lane: animated battlebacks, scene video backgrounds, title video splash/logo/social profile, and export validation.
...
5. Battle feedback lane: battler lights, blood/death/crit VFX, chip damage/healing, custom buff levels, zero-damage presentation policies, and troop-position reuse.
