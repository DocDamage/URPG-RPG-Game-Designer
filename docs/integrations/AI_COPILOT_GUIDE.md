...
`OpenAiCompatibleChatConfig` records endpoint, model, API key, request/response paths, timeout, stream preference, and dry-run/execute mode. Dry-run mode builds deterministic request bodies and curl commands without network access. Execute mode writes the request JSON, invokes curl, reads the provider response, and imports common OpenAI-compatible chat, OpenAI Responses `output_text`, Ollama-style `message.content`, and direct `{response, command}` shapes. `requestStream()` sends `stream: true`, adds curl `--no-buffer`, and imports OpenAI-compatible SSE `data:` chunks from the response file into the same response/command callback shape. Secrets are supplied at runtime by host configuration or environment resolution, not hardcoded into the engine.
