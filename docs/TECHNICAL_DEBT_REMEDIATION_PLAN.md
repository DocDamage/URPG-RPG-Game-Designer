...
1. **Visual validation hardening**
   ...
   - The backend truth story is still explicit: the real `SpriteBatcher -> renderBatches()` whole-scene path now has committed textured `MapScene` and `BattleScene` goldens, the widget-level lane now includes one committed mixed textured-plus-text `ChatWindow` proof, one bounded `EngineShell::tick()` golden now proves the top-level native loop can drive a live `MapScene` frame through both batch and frame-command routes, and direct frame-command sprite/tile submission now supports bounded preloaded-id texture resolution while unresolved sprite/tile ids still render deterministic placeholders for validation instead of disappearing entirely.
...
2. **Export pipeline hardening**
   - Keep the bounded deterministic `data.pck` container truthful: embedded manifest plus staged runtime/bootstrap payloads, narrow repo-owned content roots, and governed manifest-driven promoted-asset staging are now landed, but automatic project-wide asset discovery/bundling is still required.
   ...
   - Extend hardening beyond the current lightweight bundle-level RLE-plus-XOR plus keyed integrity-tag layer into cryptographic signing and runtime enforcement.
