...
- [x] Keep heavy ML/research dependencies isolated under `tools/` or separate helper environments.
- [x] Land the first approved offline tooling lanes: FAISS retrieval, SAM/SAM2 segmentation, and Demucs/Encodec audio tooling.
- [x] Ensure the runtime consumes only exported artifacts from those pipelines and does not silently acquire heavyweight research dependencies.
