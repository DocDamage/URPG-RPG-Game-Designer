...
> **Document status:** Ninth-pass revision — canonical remediation hub as of 2026-04-23.
...
1. **Visual validation hardening**
  - Keep the new Gate 1/local renderer-backed golden enforcement lane green.
  - Continue expanding beyond the original bounded rect/text snapshots into broader scene coverage.
  - The first follow-through slices are now landed as committed renderer-backed goldens: `MapScene` dialogue-overlay output, compat `Window_Base` status/gauge output, `ChatWindow` mixed textured-plus-text output, and one bounded `MapScene` world placeholder output are all enforced through the current OpenGL harness.
  - The backend truth story is still explicit: the real `SpriteBatcher -> renderBatches()` whole-scene path now has committed textured `MapScene` and `BattleScene` goldens, the widget-level lane now includes one committed mixed textured-plus-text `ChatWindow` proof, bounded `EngineShell::tick()` goldens now prove the top-level native loop can drive live `MapScene` and `MenuScene` frames, direct frame-command sprite/tile submission now supports bounded preloaded-id texture resolution while unresolved sprite/tile ids still render deterministic placeholders for validation instead of disappearing entirely, and the harness now exposes backend-selectable capture entry points while remaining truthful that only the OpenGL implementation is live today.
  - Broader final-form textured whole-scene/backend breadth, richer tile semantics, and wider shell-owned runtime coverage remain open work.
  - Depends on: current `VisualRegressionHarness` lane staying green.

2. **Export pipeline hardening**
  - Keep the bounded deterministic `data.pck` container truthful: embedded manifest plus staged runtime/bootstrap payloads, configured project-asset auto-discovery manifests, narrow repo-owned content roots, governed manifest-driven promoted-asset staging, and keyed SHA-256 bundle-signature validation/reporting are now landed.
  - Keep the bounded real Windows/Web/Linux/macOS bootstrap lanes truthful while expanding toward fuller native packaging/signing/notarization only when the repo can actually validate those claims.
  - Extend hardening beyond the current validator-time keyed SHA-256 bundle-signature lane into fuller runtime-side enforcement plus fuller native packaging/signing/notarization.
  - Depends on: existing Windows smoke lane and validator staying truthful.

### Priority 2 — Governance and Readiness Enforcement

3. **Release-signoff enforcement**
   - Move beyond artifact-backed signoff evidence into enforced release gates.
   - Depends on: current readiness matrices and audit outputs staying canonical.

4. **Schema/version/changelog governance**
   - Add stronger schema-version enforcement and changelog discipline.
   - Tighten cross-doc drift/date/status checks where the current first slice is only partial.
   - Depends on: current project-audit surfaces.

5. **Cross-cutting product bars**
   - Treat accessibility, localization completeness, input remapping, audio governance, and performance budgets as active delivery lanes.
   - Depends on: governance/signoff enforcement above.

### Priority 3 — Subsystem Completion Work
...
7. **Character Identity completion**
   - Expand the newly landed bounded Create-a-Character runtime workflow and preview surface into authored creation rules, runtime-created protagonist persistence, and deeper exported-game integration.
   - Expand the preview lane from the current bounded card surface into layered portrait/field/battle composition driven by appearance parts.
...
- [tools/ci/run_local_gates.ps1](../tools/ci/run_local_gates.ps1) now asserts non-zero exit codes after configure/build/test steps so broken local validation can no longer report a false-green snapshot.
- The latest full local validation snapshot on 2026-04-23 is green again: `run_local_gates.ps1` passed with `ctest` lane results of `1140/1140` (`pr`), `33/33` (`nightly`), and `47/47` (`weekly`).
...
- Do not describe `LocalInMemoryCloudService` as an integration-ready path.
...
| 2026-04-20 | Eighth-pass improvement: added Wave 1 closure signoff artifacts (`BATTLE_CORE_CLOSURE_SIGNOFF.md`, `SAVE_DATA_CORE_CLOSURE_SIGNOFF.md`), presentation bridge compilation and test registration, incubating test Catch2 conversion and CMake registration, and updated all canonical status docs to reflect 790/790 passing tests under the `dev-ninja-debug` preset.
...
| 2026-04-20 | Ninth-pass improvement: enriched VFX cue pipeline with semantic `EffectCueKind` values, kind-aware `EffectResolver` routing, new `EffectCatalog` presets, and focused integration coverage for battle-scene emission and resolver behavior. |
...
| 2026-04-21 | Sprint 02 improvement: `urpg_project_audit` and `ProjectAuditPanel` now expose explicit accessibility/audio/performance governance artifact sections and counts, extending the earlier localization/input/export audit contract with focused CLI/panel coverage. |
| 2026-04-21 | Sprint 02 improvement: `DiagnosticsWorkspace::exportAsJson()` now keeps project-audit active-tab export in parity with the richer accessibility/audio/performance governance snapshot instead of dropping those newer sections at the diagnostics boundary. |
| 2026-04-21 | Sprint 02 improvement: readiness/truth gates now validate the emitted `urpg_project_audit` governance contract and keep `PROJECT_AUDIT.md` in the canonical date/truth alignment chain, so the richer project-audit surface is enforced instead of only documented. |
| 2026-04-21 | Sprint 02 closeout: the governance-depth sprint ended with `ctest --test-dir build/dev-ninja-debug -L pr --output-on-failure`, `check_release_readiness.ps1`, and `truth_reconciler.ps1` all green, closing the planned audit-depth follow-through for this sprint. |
| 2026-04-21 | Sprint 03 improvement: added `COMPAT_BRIDGE_EXIT_SIGNOFF.md` and updated readiness/truth gates to require the compat signoff artifact alongside the existing battle/save signoff docs for human-review-gated subsystem promotion language. |
| 2026-04-21 | Sprint 03 improvement: generalized signoff-governance enforcement so all current human-review-gated subsystem lanes require matching signoff/human-review wording in readiness records, matrix rows, and signoff artifacts instead of leaving battle/save checks as prose-only. |
| 2026-04-21 | Sprint 03 closeout: the signoff-governance sprint ended with the PR lane and both readiness/truth gates green, closing the planned release-signoff discipline follow-through for the current governed subsystem set. |
| 2026-04-21 | Sprint 04 improvement: added `RELEASE_SIGNOFF_WORKFLOW.md` and updated readiness/truth gates to require the canonical release-signoff workflow artifact alongside the existing audit/signoff stack, narrowing the remaining `governance_foundation` gap without inventing machine-written approvals. |
| 2026-04-21 | Sprint 04 improvement: `urpg_project_audit`, `ProjectAuditPanel`, and `DiagnosticsWorkspace` now carry a dedicated `releaseSignoffWorkflow` governance section plus issue count so the project-audit contract stays in parity with the canonical release-signoff workflow artifact. |
| 2026-04-21 | Sprint 09 improvement: `ProjectAuditPanel` and `DiagnosticsWorkspace` now preserve the richer nested `signoffArtifacts` and `templateSpecArtifacts` payloads already emitted by `urpg_project_audit`, including expected-artifact rows, structured signoff-contract state, and template-parity drift details instead of flattening those sections to coarse summaries. |
| 2026-04-21 | Sprint 10 improvement: the export validation lane now has sharper executable proof: `ExportPackager::runExport()` fails fast on invalid export roots, focused validator/packager tests share synthetic target fixtures, and `ExportDiagnosticsPanel` now reflects the same packager preflight seam instead of drifting from it. |
| 2026-04-22 | Export hardening improvement: `ExportPackager::validateBeforeExport()` now performs only real preflight/staging checks, while `ExportValidator::validateExportDirectory()` remains post-export artifact validation. Focused packager and diagnostics tests now prove fresh empty output roots can be ready to export before emitted-tree validation passes. |
| 2026-04-22 | Export hardening improvement: `ExportPackager::bundleAssets()` now writes a deterministic bounded `data.pck` container with an embedded manifest plus staged export/runtime payloads, optionally compressed through `ResourceProtector`; focused tests now prove the bundle is non-placeholder, deterministic for identical inputs, and reflected honestly through diagnostics even when preflight and post-export states diverge. |
| 2026-04-22 | Export hardening improvement: `ExportPackager::synthesizeExecutable()` now emits a bounded real `Web_WASM` bootstrap made of a valid empty Wasm module, HTML entry document, and JS loader that fetches `data.pck` plus `game.wasm`, narrowing the non-Windows synthesis gap without pretending Linux/macOS launchable outputs are already solved. |
| 2026-04-22 | Export hardening improvement: `ExportPackager::synthesizeExecutable()` now emits a bounded real `Linux_x64` launcher script that validates `data.pck` presence and provides a launchable executable-shaped bootstrap contract instead of a fake ELF marker string, narrowing the remaining non-Windows synthesis gap to macOS only. |
| 2026-04-22 | Export hardening improvement: `ExportPackager::synthesizeExecutable()` now emits a bounded real `macOS_Universal` app bootstrap with `Contents/Info.plist` plus a `Contents/MacOS/MyGame` launcher script that validates `data.pck` presence, closing the fake Mach-O marker seam without overstating native packaging/signing/notarization readiness. |
| 2026-04-22 | Export hardening improvement: `ExportPackager::bundleAssets()` now applies the shipped XOR obfuscation layer to protected bundle payloads in addition to RLE compression and records that state in the bundle manifest, closing the silent gap where the in-tree export path claimed lightweight protection but only exercised compression. |
| 2026-04-22 | Export hardening improvement: `ExportPackager::bundleAssets()` now stages a bounded repo-owned content set into `data.pck` alongside export/bootstrap metadata, including the readiness record, content schemas, and level-library JSON roots; focused tests now prove those entries are present and decodable without overstating the bundle as generic project asset discovery. |
| 2026-04-22 | Export hardening improvement: `ExportPackager` now records keyed FNV-1a integrity tags for bundled payloads, and `ExportValidator` now verifies those tags during post-export validation so `data.pck` corruption or simple tampering is surfaced explicitly instead of counting any existing bundle file as valid. |
| 2026-04-22 | Export breadth improvement: `ExportPackager` now supports governed manifest-driven promoted-asset staging from `imports/manifests/asset_bundles/*.json`, bundling only `promoted` entries that resolve to existing repo-local assets under `imports/normalized/`, without promoting that bounded seam into automatic project-wide asset discovery. |
| 2026-04-21 | Sprint 11 improvement: `tools/ci/check_localization_consistency.ps1` now emits a canonical localization consistency report and `urpg_project_audit` consumes that bounded artifact as `localizationEvidence`, so missing-key drift can surface through executable governance without widening the audit into a full localization crawler. |
| 2026-04-21 | Sprint 12 improvement: the readiness and truth gates now enforce the richer shipped `urpg_project_audit` governance contract, requiring `localizationEvidence`, `signoffArtifacts`, and `templateSpecArtifacts` plus their top-level issue counts and the canonical localization-consistency-report wording in `PROJECT_AUDIT.md`. |
| 2026-04-21 | Sprint 12 improvement: `ProjectAuditPanel` and `DiagnosticsWorkspace` now keep bounded `localizationEvidence` in parity with the CLI contract, including the issue count, status, bundle counters, master locale, and bundle payload instead of dropping that governance section at the diagnostics boundary. |
| 2026-04-23 | TD Sprint 01 improvement: `project.schema.json` now defines the canonical project governance sections `localization`, `input`, and `exportProfiles`, with a fixture and ProjectAudit regressions proving positive, missing, and malformed schema-governance cases. |
| 2026-04-23 | TD Sprint 01 improvement: ProjectAudit localization artifact governance now checks the shipped `localization_bundle` / `LocaleCatalog` / `CompletenessChecker` / consistency-gate contract instead of stale localization-catalog extract/writeback paths. |
| 2026-04-23 | TD Sprint 02 improvement: `tools/pack/pack_cli.cpp` now provides the canonical bounded export CLI with JSON preflight/export/post-export validation phases, and focused process-level tests prove both successful Web export and invalid output-root failure behavior. |
| 2026-04-23 | TD Sprint 02 release-truth pass: battle/save validation evidence was refreshed, but `battle_core` and `save_data_core` remain `PARTIAL` with two release blockers until human reviewers record explicit accept/reject notes in the signoff workflow. |
| 2026-04-23 | TD Sprint 03 improvement: the compat runtime contract remains explicitly fixture-backed, while `Window_Base` now parses JS color objects, arrays, hex strings, and rgb/rgba strings, emits segmented gauge-gradient fills, and keeps `contents()` handle/dimension-backed with focused window/QuickJS/plugin regressions instead of claiming live runtime/backend parity. |
| 2026-04-23 | TD Sprint 04 improvement: asset intake now has one promoted visual lane and one promoted UI-audio lane with source snapshots, normalized assets, bundle manifests, WYSIWYG smoke reporting, ProjectAudit regression checks, and export bundle staging; Gameplay Ability async depth now includes deterministic WaitInput, WaitEvent, and WaitProjectileCollision task backends while scripted conditions remain intentionally unsupported. |
| 2026-04-23 | TD Sprint 05 improvement: `profile_arena.cpp` is now registered as the standalone `urpg_profile_arena` target instead of a CMake-completeness exception, the renderer-backed lane now carries a shell-owned BattleScene actor-cue crop golden on the real OpenGL backend, the two largest compat test surfaces have first domain splits, and `urpg_project_audit` now uses a helper-extracted asset-report summary parser. |
| 2026-04-21 | Sprint 04 closeout: the release-signoff workflow sprint ended with the focused audit/diagnostics lane, the readiness/truth gates, and the full PR lane green. |
| 2026-04-21 | Sprint 08 improvement: the governed human-review subsystem lanes now carry a structured `signoff` contract in `readiness_status.json`, and the readiness/truth gates plus `urpg_project_audit` enforce that contract alongside the existing signoff-artifact presence and conservative wording checks. |
| 2026-04-21 | Sprint 08 improvement: `test_visual_regression_harness.cpp` now proves the approval-script path produces harness-consumable golden files, and `tools/ci/check_visual_regression_harness.ps1` adds a focused executable gate for the file-backed visual-regression harness contract without overclaiming real render capture. |
| 2026-04-21 | Sprint 07 improvement: `urpg_project_audit` now enforces conservative parity between the selected template spec artifact and the readiness record for authority line, required subsystem rows, and cross-cutting bar rows; focused CLI regressions cover bar drift and required-subsystem drift, and the `monster_collector_rpg` / `2_5d_rpg` specs were reconciled to the current canonical readiness truth. |
...
| 2026-04-23 | Debt follow-through improvement: moved the last vendored SDL configure-time deprecation noise out of local validation by setting the shared CMake preset warning policy to suppress deprecation warnings, keeping `run_local_gates.ps1` clean while preserving the repo-owned warnings-as-errors lane. |
...
| 2026-04-22 | Debt follow-through improvement: tightened TD-06/TD-09/TD-10/TD-11 truth surfaces by replacing fake `ResourceProtector::compress()` passthrough with lightweight real RLE compression, surfacing `active_condition_unsupported` diagnostics in GAF, marking AI/cloud seams as simulated/not live, and replacing CopilotKernel substring-based canon checks with predicate-based constraints. |
...
| 2026-04-22 | Debt follow-through improvement: `VisualRegressionHarness` now has a bounded OpenGL-enabled renderer-backed capture lane through `urpg_snapshot_tests` plus committed clear-frame, full-frame-rect, and inset-rect goldens, with focused enforcement in both local gates and Gate 1 CI. Live-surface follow-through is also landed through committed renderer-backed `MapScene` dialogue-overlay crop, compat `Window_Base` status/gauge crop, and `ChatWindow` mixed textured-plus-text crop goldens. The OpenGL backend no longer silently drops frame-command tile/sprite submissions in this lane because unresolved sprite/tile ids now render deterministic placeholder quads backed by a committed `MapScene` world placeholder golden, preloaded logical sprite and tile ids now have committed direct textured frame-command goldens, the real `SpriteBatcher -> renderBatches()` path now has committed textured `MapScene` and `BattleScene` goldens as truthful whole-scene runtime proofs, and one bounded `EngineShell::tick()` golden now proves a live shell-owned `MapScene` frame through the same capture lane. The remaining visual-validation gap is broader final-form whole-scene/backend breadth rather than missing CI enforcement or live-surface proof entirely. |
| 2026-04-22 | Debt follow-through improvement: completed the current TD-07 warning burn-down in repo code, quarantined vendored SDL warning noise at the SDL target, and resolved the stale `dev-ninja-debug` Ninja-state warning by regenerating the corrupted `.ninja_log`. |
| 2026-04-22 | Debt follow-through improvement: made `AssetLoader` explicitly single-thread-affine with guarded cache access and cross-thread rejection, completed the native compat sprite source-rect/motion semantics pass, and narrowed the incubator editor seam before later retiring it entirely. |
| 2026-04-22 | Debt follow-through improvement: finished the safe TD-10 keep/cut truthfulness pass by collapsing the in-tree cloud provider enum to `LocalSimulated`, documenting the active chat path as `IChatService` plus deterministic `MockChatService`, and removing remaining in-tree production-looking provider aliases from the canonical docs. |
| 2026-04-22 | Debt follow-through improvement: retired the dead `EngineAssembly` / `MainAssembly` top-level incubator seam and its runtime alias headers. |
| 2026-04-22 | Debt follow-through improvement: retired the dead editor-side incubator `ScriptBridge` / `PluginHost` / `ScriptingConsole` seam from `tools/incubator/editor/`, so the active compat harness is now the only remaining scripting/plugin harness path in tree. |
