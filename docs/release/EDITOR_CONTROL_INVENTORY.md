...
| `Nested` | `event_authoring`, `quest`, `character_creator`, `database`, `vendor`, `battle_preview`, `audio_mix`, `accessibility`, `input_remap`, `export_diagnostics` | Explicitly registered Map-context routes for the bounded M6-M9 creator workflow. They remain nested rather than independent release navigation and still require M10/M11 manual qualification. |
