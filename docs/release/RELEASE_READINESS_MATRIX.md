...
| `native_level_builder` | `READY` | Yes | Yes | Yes | Yes | Yes | Yes | `LevelBuilderWorkspace` owns grid-part build/validate/playtest/package workflows, save/load/export commands, undo/redo, actionable diagnostics, diagnostic focus, spawn/objective authoring, readiness evidence commands, package readiness, certified export preflight, and first-class Perspective 2D handoff. The grid-part runtime/data model includes schemas, catalog/document/serializer/runtime compiler, reachability, dependency graph, package governance, runtime state, CTest unit/integration lanes, focused editor tests, app-shell top-level navigation, and smoke coverage for `level_builder` plus the top-level `spatial_authoring` Perspective 2D route. The Perspective 2D workspace composes elevation, props, parts, ability bindings, terrain brush preview, region rules, procedural generation preview, and map environment/runtime preview into one map-authoring surface. The P2D depth slice now adds `tiles` mode, promoted asset tile palette rows, palette search plus tileset/category filters, tile thumbnail/category preview metadata, selected palette option and tileset/tile state, brush-size painting/erasing, RPG Maker-style tileset page metadata for pages A-Z, autotile metadata, animated tile frames, passage/collision flags, terrain tags, region IDs, priority/star passability, coordinate tile previews, visible/locked layer rows, single-layer and multi-layer selection, bulk layer visible/locked toggles, layer duplicate/clear/delete operations, duplicate/delete selected-layer operations, event/object rows, event movement, editable/removable event command rows, RPG Maker-style event pages, page reorder/duplicate/delete operations, editable/removable page conditions and page commands, variable/switch comparison condition rules, conditional branch command rows with nested true/false child commands, active-page preview state, event execution preview traces, live runtime execution for the supported command set (`show_text`, `transfer_player`, switch/variable/self-switch mutation, gold/item changes, movement routes, common events, and conditional branches), persistent runtime state snapshots, project/database references for actors/items/switches/variables/common events/maps/starting party/transfers/encounters/assets/save-load profiles, RPG-editor UI model snapshots for page tabs, condition editor, command list, branch tree, command picker, palette/layer panels, playtest controls, and export controls, playtest execution handoff trace bundles, JSON draft save/load, runtime playtest manifests, deterministic package inventory/signature proof, export package manifests, creator UX readiness summaries, release asset/LFS policy gate results, playtest readiness diagnostics, export readiness snapshots, and a `Perspective2DProductWorkflow` analyzer exposed through `RenderSnapshot::perspective_2d_product` that validates draft/runtime/export contracts for supported command coverage, transfer edges, package files, package signatures, and product blockers. | Public product release remains governed by app-level release/tagging gates; P2D depth and product workflow analysis are bounded to this durable authoring/runtime slice rather than blanket full RPG Maker parity. |
...
| Current checkout LFS footprint | `RECONCILED_FOR_BOUNDED_SCOPE` | On 2026-05-27, `.\tools\ci\check_lfs_release_scope.ps1` reports 307,388 LFS-tracked paths, zero release-required LFS paths, and zero unknown LFS paths outside the governed allowlist. `.\tools\ci\check_promoted_asset_library.ps1` validates the full promoted manifest inventory. The machine-readable reports are `imports/reports/asset_intake/lfs_release_scope_report.json` and `imports/reports/asset_intake/promoted_asset_library_report.json`, and the human-readable scope note is `docs/asset_intake/LFS_SCOPE_RECONCILIATION.md`. | Do not claim current-branch zero-LFS. Governed promoted-library LFS payloads are validated inventory; future release scopes that depend on optional LFS packs must hydrate exact project selections before making broader claims. |
| Current checkout generated/root path hygiene | `VERIFIED` | `.\tools\ci\check_no_generated_tracked_files.ps1` guards generated/local retired-root tracking. | Keep the guard in local gates so generated/local roots cannot regress into tracking. |
