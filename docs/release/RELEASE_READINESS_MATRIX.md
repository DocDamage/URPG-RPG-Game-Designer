...
| Current checkout LFS footprint | `RECONCILED_FOR_BOUNDED_SCOPE` | On 2026-05-27, `.\tools\ci\check_lfs_release_scope.ps1` reports 307,388 LFS-tracked paths, zero release-required LFS paths, and zero unknown LFS paths outside the governed allowlist. The machine-readable report is `imports/reports/asset_intake/lfs_release_scope_report.json`, and the human-readable scope note is `docs/asset_intake/LFS_SCOPE_RECONCILIATION.md`. | Do not claim current-branch zero-LFS. Deferred curated-library LFS payloads are governed inventory, not final-art/content-complete proof; future release scopes that depend on optional LFS packs must hydrate exact selections before making broader claims. |
| Current checkout generated/root path hygiene | `VERIFIED` | `git ls-files build-local Testing/Temporary third_party "itch/loose"` returns no tracked paths; `.\tools\ci\check_no_generated_tracked_files.ps1` passes after removing those roots from the Git index. | Keep the guard in local gates so generated/local roots cannot regress into tracking. |
