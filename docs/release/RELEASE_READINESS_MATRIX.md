...
| Current checkout LFS footprint | `GATED_FOR_BOUNDED_SCOPE` | On 2026-05-26, `git lfs ls-files --name-only` reports 32,229 LFS-tracked normalized asset paths in this checkout. P2D release asset gate metadata now records release-required assets separately from optional deferred LFS payloads, so bounded P2D package readiness does not claim current-branch zero-LFS. | Do not claim current-branch zero-LFS. Future release scopes that depend on optional LFS packs must hydrate, remove, or reclassify those payloads before making broader release claims. |
