...
| `gameplay_ability_framework` | `PARTIAL` | Yes | Yes | Partial | Yes | Yes | Yes | Gameplay ability runtime, inspector/editor surfaces, structured diagnostics snapshot, end-to-end release-grade test, closure signoff artifact, gameplay_ability JSON schema (S24-T01), compat-to-native MZ mapper (S24-T02), live BattleScene activation, AbilityBattleQueue integration (S24-T03/T04), deterministic AbilityTask WaitInput/WaitEvent/WaitProjectileCollision backends, and authored battle/map ability orchestration with `AbilityOrchestrationPanel` preview are landed. | One authored battle/map orchestration path is landed; scripted condition evaluator and broader task-driven authored composition remain mandatory current technical backlog. Wave 1 closure signoff exists, and release-owner review accepted manual residual-gap review for the current internal branch. |
