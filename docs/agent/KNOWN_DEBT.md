...
- M6 is the next creator-loop dependency: editor-owned unsaved current-map playtest, bounded diagnostics, explicit targets, process ownership, and return without discarding editor state.
- M7 must add ignored, bounded, atomic recovery snapshots and explicit asset relinking without overwriting manual saves or relinking to raw external sources.
...
- M9-M10 must prove the governed vertical slice and then complete manual graphical, accessibility, input, and reference-machine performance review. Headless/model coverage is not manual signoff.
...
- PFU-I1 now has a separate target qualification contract in `content/fixtures/creator_journey_qualification_spec.json` and `tools/ci/check_creator_journey_qualification.ps1`. It is intentionally stricter than the historical creator-journey baseline and remains unsatisfied until every target step has clean, commit-matched Debug/Release evidence.
