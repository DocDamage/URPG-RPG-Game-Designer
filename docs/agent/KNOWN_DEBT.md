...
- Treat M0-M5 as a bounded implemented foundation, not a completed product. Perspective 2D and the Character Creator draft now register with the shared dirty-state owner; remaining M1/M3/M4 breadth includes other durable non-Map authoring surfaces, extending the atomic Map Tiles drop beyond its current immediate placement path (Props remains palette enrollment) to other contextual owners, completing shortcut coverage, and recording graphical route/layout equivalence.
