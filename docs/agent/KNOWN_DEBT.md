...
- M7 now has checksummed, ignored snapshot primitives, orderly-session markers, five-minute bounded private capture of dirty Grid Parts, Perspective 2D, ability, Character Creator, and quest drafts, plus in-editor count, safe restore-to-new-folder, and clean-session open-recovered-project actions. It remains incomplete: broader durable-surface coverage, explicit governed-asset relinking, and a creator walkthrough.
- M8 has initial Character Creator, ability, event, and quest slices: Map opens the active project’s durable character draft or Ability Inspector without losing Map context; its Perspective 2D event form authors an event, first page, and optional `show_text` dialogue command in the normal Map document/save/recovery path; and its quest form authors, previews, saves, and recovers a typed objective graph. Each authored event exposes its existing native preview/runtime execution controls. Database/battle/audio contextual authoring and the WYSIWYG done rule remain open. The Capybara repository is `reference_only`; any adopted concept needs URPG-native ownership, provenance/license evidence, deterministic tests, and no parallel browser/npm/cloud/save/pathfinding stack.
