...
- M6 now has an initial native session foundation: an editor-owned runtime child, private current-map overlay, authored player-start or selected-part target (with blank-starter fallback), explicit map/spawn/session manifest, F5 stop/restart, document/catalog launch blocking, JSONL diagnostics ingestion, and return without discarding editor state. It remains incomplete: broader runtime blocker policy, map-entrance/checkpoint targets, diagnostic focus, and safe hot reload are still open.
