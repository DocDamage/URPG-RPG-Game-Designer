...
- Treat M0-M5 as a bounded implemented foundation, not a completed product. Remaining M1/M3/M4 breadth is explicit: aggregate Perspective 2D-only changes and register durable non-Map authoring surfaces with the shared dirty-state owner, extend the atomic Map Tiles drop beyond its current immediate placement path (Props remains palette enrollment) to other contextual owners, complete shortcut coverage, and record graphical route/layout equivalence.
...
- M7 now has checksummed, ignored snapshot primitives, orderly-session markers, five-minute bounded private capture of dirty Grid Parts, Perspective 2D, and ability drafts, plus in-editor count, safe restore-to-new-folder, and clean-session open-recovered-project actions. It remains incomplete: broader durable-surface coverage, explicit governed-asset relinking, and a creator walkthrough.
