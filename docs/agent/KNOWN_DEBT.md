...
- M7 now has checksummed, ignored snapshot primitives, orderly-session markers, five-minute bounded private capture of dirty Grid Parts, Perspective 2D, ability, Character Creator, quest, database, vendor, and audio-mix drafts, plus in-editor count, safe restore-to-new-folder, and clean-session open-recovered-project actions. It remains incomplete: broader durable-surface coverage, explicit governed-asset relinking, and a creator walkthrough.
- M8 has initial Character Creator, ability, event, quest, database, vendor, battle-preview, and audio-mix slices: Map opens the active project's durable character draft or Ability Inspector without losing Map context; its Perspective 2D event form authors an event, first page, and a supported native command (text/choice dialogue, transfer, state, inventory, common event, encounter launch, or vendor open) in the normal Map document/save/recovery path; its event runtime feeds switch, variable, inventory, and dialogue-choice state into the contextual quest preview, while an encounter launch opens the native Battle Inspector with the active character and ability draft queued and a vendor-open command resolves the native vendor stock preview by stable ID; a marked Battle Inspector preview victory feeds the quest preview only and is explicitly not a playtest combat claim; its contextual encounter control provides the same direct preview route; its contextual audio control applies and saves a project mix preset through the native audio core and assigns an attached audio asset with owner-local undo/redo; its quest form authors, previews, saves, and recovers a typed objective graph; and its database/vendor form authors a project item and database-validated vendor stock. Each authored event exposes its existing native preview/runtime execution controls. Battle-result, broader audio cue routing, and the WYSIWYG done rule remain open. The Capybara repository is `reference_only`; any adopted concept needs URPG-native ownership, provenance/license evidence, deterministic tests, and no parallel browser/npm/cloud/save/pathfinding stack.
