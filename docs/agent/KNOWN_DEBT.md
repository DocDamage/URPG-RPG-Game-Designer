...
- M7 now has checksummed, ignored snapshot primitives, orderly-session markers, five-minute bounded private capture of dirty Grid Parts, Perspective 2D, ability, Character Creator, quest, database, and vendor drafts, plus in-editor count, safe restore-to-new-folder, and clean-session open-recovered-project actions. It remains incomplete: broader durable-surface coverage, explicit governed-asset relinking, and a creator walkthrough.
- M8 has initial Character Creator, ability, event, quest, database, vendor, and battle-preview slices: Map opens the active project's durable character draft or Ability Inspector without losing Map context; its Perspective 2D event form authors an event, first page, and a supported native command (dialogue, transfer, state, inventory, common event, or encounter launch) in the normal Map document/save/recovery path; its contextual encounter control opens the native Battle Inspector with an authored encounter ID and queued ability preview; its quest form authors, previews, saves, and recovers a typed objective graph; and its database/vendor form authors a project item and database-validated vendor stock. Each authored event exposes its existing native preview/runtime execution controls. Battle-result and audio contextual authoring plus the WYSIWYG done rule remain open. The Capybara repository is `reference_only`; any adopted concept needs URPG-native ownership, provenance/license evidence, deterministic tests, and no parallel browser/npm/cloud/save/pathfinding stack.
