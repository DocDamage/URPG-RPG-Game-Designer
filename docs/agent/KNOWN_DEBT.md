...
- M8 has initial Character Creator, ability, event, quest, database, and vendor slices: Map opens the active project’s durable character draft or Ability Inspector without losing Map context; its Perspective 2D event form authors an event, first page, and optional `show_text` dialogue command in the normal Map document/save/recovery path; its quest form authors, previews, saves, and recovers a typed objective graph; and its database/vendor form authors a project item and database-validated vendor stock. Each authored event exposes its existing native preview/runtime execution controls. Battle/audio contextual authoring and the WYSIWYG done rule remain open. The Capybara repository is `reference_only`; any adopted concept needs URPG-native ownership, provenance/license evidence, deterministic tests, and no parallel browser/npm/cloud/save/pathfinding stack.
