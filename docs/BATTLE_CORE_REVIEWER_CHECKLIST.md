...
```powershell
.\build\dev-ninja-debug\urpg_tests.exe "[battle][core]" --reporter compact
```
...
```powershell
.\build\dev-ninja-debug\urpg_tests.exe "[battle][migration]" --reporter compact
```
...
```powershell
.\build\dev-ninja-debug\urpg_tests.exe "[battle]" --reporter compact
...
Automation has completed all evidence-refresh work it can truthfully perform for `battle_core`. The reviewer still must make the accept/reject decision, but no unchecked machine-validation command remains in this checklist.

| Checklist Area | Command | Automated Result |
|---|---|---|
| Runtime correctness | `.\build\dev-ninja-debug\urpg_tests.exe "[battle][core]" --reporter compact` | Passed: 39 assertions / 5 test cases |
| Migration and compat coverage | `.\build\dev-ninja-debug\urpg_tests.exe "[battle][migration]" --reporter compact` | Passed: 207 assertions / 35 test cases |
...
| Integration and cross-subsystem | `.\build\dev-ninja-debug\urpg_tests.exe "[battle]" --reporter compact` | Passed: 611 assertions / 88 test cases; emitted expected `MISSING_BATTLEBACK` diagnostics from fallback-asset coverage |
...
Machine conclusion: `battle_core` has current machine evidence prepared for human review. Automation cannot mark residual gaps accepted, cannot select `READY`, and cannot update `readiness_status.json` without a human reviewer decision.

### Automated Evidence Refresh - 2026-04-24

Automation refreshed the battle evidence without granting human approval:

- `.\build\dev-ninja-debug\urpg_tests.exe "[battle]" --reporter compact` passed: 611 assertions / 88 test cases.
...
- `.\build\dev-ninja-debug\urpg_project_audit.exe --json` still reports `releaseBlockerCount: 2`, with `battle_core` intentionally blocked on explicit human review.
