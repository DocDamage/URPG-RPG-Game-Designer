 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, this version adds the ability to display a video on your profession
 * display info window. Each profession can have its own video, and you need
 * to also tell the plugin the width and height of the video. This video will
 * play silently on loop, just like how a gif would work. You can use this to
 * give the player a visual of what the profession can do.
 *
 * Version 1.9.0
 ...
 * @type file
 * @dir movies/
 * @desc A video that can be shown on the profession info window
 *
 * @param Video Width
 * @type number
 * @default 0
...
CGMZ.Versions["Professions"] = "1.9.0";
CGMZ.Professions = {};
...
CGMZ_ProfessionTemp.prototype.initialize = function(profession) {
	...
	this.video = profession.Video;
	this.videoWidth = Number(profession["Video Width"]);
	this.videoHeight = Number(profession["Video Height"]);
...
//-----------------------------------------------------------------------------
// Also clear any active videos
//-----------------------------------------------------------------------------
CGMZ_Window_ProfessionDisplay.prototype.destroy = function() {
	CGMZ_Window_Scrollable.prototype.destroy.call(this);
	this.clearVideos();
};
//-----------------------------------------------------------------------------
// Clear all active videos
//-----------------------------------------------------------------------------
CGMZ_Window_ProfessionDisplay.prototype.clearVideos = function() {
	for(const video of this._inlineVideos) {
		video.remove();
	}
	for(const sprite of this._inlineVideoSprites) {
...
CGMZ_Window_ProfessionDisplay.prototype.update = function() {
	...
	for(let i = 0; i < this._inlineVideos.length; i++) {
		const sprite = this._inlineVideoSprites[i];
		const vid = this._inlineVideos[i];
		if(!sprite || !vid) break;
		sprite.bitmap.clear();
		sprite.bitmap.context.drawImage(vid._element, 0, 0, sprite.bitmap.width, sprite.bitmap.height);
...
CGMZ_Window_ProfessionDisplay.prototype.drawProfessionInfo = function() {
	...
	for(const infoType of CGMZ.Professions.DisplayInfo) {
		switch(infoType) {
			...
			case 'Video': this.drawProfessionVideo(); break;
			case "Custom Info":
...
// Add a video
//-----------------------------------------------------------------------------
CGMZ_Window_ProfessionDisplay.prototype.drawProfessionVideo = function() {
	const prof = $cgmzTemp.getProfessionTempData(this._profession._name);
	if(!prof.video) return;
	const video = new CGMZ_DisposableVideo(prof.videoWidth, prof.videoHeight, `profession-video`);
	video.play(prof.video);
	const sprite = new Sprite(new Bitmap(prof.videoWidth, prof.videoHeight));
	this._inlineVideos.push(video);
	this._inlineVideoSprites.push(sprite);
	...
	this._neededHeight += (sprite.height * sprite.scale.y);
};
//=============================================================================
// Spriteset_Map
//-----------------------------------------------------------------------------
