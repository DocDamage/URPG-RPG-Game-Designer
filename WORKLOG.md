...
- **Action**: Continued TD-AUD-06 by splitting curated compat fixture scenarios by profile family: content/save/library flows moved to `test_compat_plugin_fixtures_content.cpp`, menu/presentation/reload flows moved to `test_compat_plugin_fixtures_menu_presentation.cpp`, and the original fixture DSL/orchestration file dropped to 1,367 lines.
- **Action**: Continued the safer mixed-chain paydown inside `test_compat_plugin_failure_mixed_chain.cpp` by extracting named setup helpers for curated happy-path fixture loading, dependency-gate probe verification, weekly lifecycle fixture exercise, diagnostic operation counts, dependency-gate diagnostic rows, runtime diagnostic rows, and load-plugin diagnostic rows while preserving the single aggregate diagnostic-stream assertion.
- **Action**: Split sprite-specific compat code out of `runtimes/compat_js/window_compat.cpp` into `runtimes/compat_js/window_sprite_compat.cpp`, moving tracked sprite bitmap handles plus `Sprite_Character` and `Sprite_Actor` implementations into their own translation unit.
- **Validation**: `cmake --build --preset dev-debug --target urpg_tests` passed; diagnostics filters passed for `[editor][diagnostics][integration]` (1,146 assertions / 47 test cases), `[menu_parity]` (22 / 2), `[message_actions]` (49 / 4), and `[battle_preview]` (15 / 1). `check_ctest_discovery.ps1` discovered 1,249 tests, `check_cmake_completeness.ps1` passed, and the exact PR CTest lane passed 1,167/1,167.
- **Validation**: `cmake --build --preset dev-debug --target urpg_compat_tests` passed; `urpg_compat_tests "[compat][fixtures]"` passed 3,449 assertions / 44 test cases; `ctest --test-dir build\dev-ninja-debug -L weekly --output-on-failure` passed 47/47; CMake completeness and CTest discovery remained green.
- **Validation**: The focused mixed-chain weekly case passed after setup helper extraction and again after runtime/load diagnostic helper extraction: `urpg_compat_tests "[compat][fixtures][failure][weekly]"` => 533 assertions / 1 test case. The full weekly CTest lane passed 47/47 after the final tail-helper pass.
- **Validation**: After the sprite compat split, `cmake --build --preset dev-debug --target urpg_tests urpg_compat_tests`, `urpg_tests "[window]"` (421 assertions / 75 test cases), the focused mixed-chain weekly case, `ctest --test-dir build\dev-ninja-debug -L weekly --output-on-failure` (47/47), `check_cmake_completeness.ps1`, and `check_ctest_discovery.ps1 -BuildDirectory build\dev-ninja-debug` all passed.
- **Result**: The diagnostics workspace and curated compat fixture review surfaces are materially smaller without reducing coverage, the mixed-chain test now has named setup and diagnostics phases, and `window_compat.cpp` dropped from 2,492 to 2,139 lines with the 371-line sprite implementation isolated in `window_sprite_compat.cpp`.

### 2026-04-24 - Technical Debt Audit Sprint 5: Partial Product Lane Vertical Slices
- **Action**: Converted the TD-AUD-05 partial product lanes into `content/readiness/partial_lane_vertical_slices.json`, giving each audit-listed lane an owner track, next vertical slice, and explicit deferred scope.
- **Action**: Advanced the first product-facing slice for `achievement_registry`: `AchievementRegistry::exportTrophyPayload()` now emits a vendor-neutral trophy payload, `AchievementPanel` exposes its summary, and the governed contract is documented through `content/schemas/achievement_trophy_export.schema.json` plus `content/fixtures/achievement_trophy_export_fixture.json`.
- **Action**: Extended achievement governance and readiness truth: `check_achievement_governance.ps1` validates the new schema/fixture, `docs/SCHEMA_CHANGELOG.md` records the additive schema, and canonical readiness docs now state that only platform-specific achievement backend integration remains out-of-tree.
- **Validation**: `check_achievement_governance.ps1`, `urpg_tests "[achievement]"` (154 assertions / 42 test cases), `urpg_tests "[wysiwyg][achievement]"` (14 assertions / 3 test cases), `urpg_tests "[wysiwyg][partial_lanes][td_aud_05]"` (44 assertions / 1 test case), release-readiness, truth-reconciler, visual golden governance, `urpg_project_audit --json`, and the exact `pr` CTest lane (1167/1167) all passed.
- **Result**: TD-AUD-05 has an owned vertical-slice map and one real product-facing slice landed without promoting any subsystem or template to `READY`; `urpg_project_audit --json` still reports `releaseBlockerCount: 2` and `exportBlockerCount: 0`, preserving the human-review gate.

### 2026-04-24 - Technical Debt Audit Sprint 4: Visual Golden Size Governance
- **Action**: Closed TD-AUD-04 as a governance sprint by adding `content/fixtures/visual_golden_governance.json`, which explicitly lists every committed renderer-backed `*.golden.json` above 10 MiB with a reason, review strategy, human-review artifact or summary, and per-file `maxBytes` ceiling.
- **Action**: Added `tools/ci/check_visual_golden_governance.ps1` and wired it into the local and Gate 1 CI paths so future oversized JSON goldens cannot appear silently; the guard also verifies the current JSON text policy and future PNG/LFS policy in `.gitattributes`.
- **Action**: Added a `[testing][visual_regression]` regression in `tests/unit/test_visual_regression_harness.cpp` and documented the storage decision in `docs/presentation/VISUAL_GOLDEN_GOVERNANCE.md`.
- **Validation**: `check_visual_golden_governance.ps1` passed with 5 governed oversized goldens, `urpg_tests "[testing][visual_regression]"` passed 96 assertions / 10 test cases, `check_visual_regression_harness.ps1` passed, `check_renderer_backed_visual_capture.ps1 -BuildDirectory build\dev-ninja-debug` passed 104 assertions / 13 test cases, presentation doc links passed, and readiness/truth governance remained green.
- **Result**: Existing large JSON baselines remain executable and governed without a churn-only format migration. Future large visual baselines now require deliberate manifest review or a compact PNG/binary migration.

### 2026-04-24 - Technical Debt Audit Sprint 3: Export Runtime Enforcement Boundary
- **Action**: Closed TD-AUD-03 as a design/readiness-boundary sprint by adding `docs/EXPORT_RUNTIME_SIGNATURE_ENFORCEMENT_DESIGN.md`, which defines the missing runtime/load-time `data.pck` signature-enforcement contract before any export-validator `READY` promotion.
- **Action**: Updated readiness/status wording to keep the distinction explicit: the current keyed SHA-256 bundle-signature lane is validator-time protection, while runtime tamper rejection, temp-file-plus-atomic-rename bundle publication, full native signing, and notarization remain backlog.
- **Action**: Added a governance regression in `tests/unit/test_s32_wysiwyg_lanes.cpp` so the runtime-enforcement design note must keep the validator-time boundary, runtime loader requirement, atomic publication decision, and out-of-scope signing claims visible.
- **Result**: No export readiness promotion was made. The sprint now has a concrete pre-READY contract and test plan for future runtime-side bundle tamper rejection.

### 2026-04-24 - Technical Debt Audit Sprint 2: Battle/Save Signoff Evidence Refresh
- **Action**: Refreshed TD-AUD-02 evidence for `battle_core` and `save_data_core` without granting automated approval. Updated the reviewer checklists so their verification commands use actual Catch2 tag filters instead of `ctest -R` source-file-name patterns that discover zero tests.
- **Validation**: Battle evidence passed: `.\build\dev-ninja-debug\urpg_tests.exe "[battle][core]" --reporter compact` => 39 assertions / 5 test cases, `"[battle][migration]"` => 207 assertions / 35 test cases, and `"[battle]"` => 611 assertions / 88 test cases.
- **Validation**: Save evidence passed: `.\build\dev-ninja-debug\urpg_tests.exe "[save]" --reporter compact` => 509 assertions / 62 test cases, `"[save][migration]"` => 49 assertions / 5 test cases, and `"[save][schema]"` => 87 assertions / 6 test cases.
- **Validation**: Cross-subsystem evidence passed: `.\build\dev-ninja-debug\urpg_integration_tests.exe "[battle][save]" --reporter compact` => 44 assertions / 4 test cases and `"[integration][wave1][closure]"` => 39 assertions / 4 test cases. `check_save_policy_governance.ps1`, `check_release_readiness.ps1`, and `truth_reconciler.ps1` also passed.
- **Result**: `.\build\dev-ninja-debug\urpg_project_audit.exe --json` still reports `releaseBlockerCount: 2` and `exportBlockerCount: 0`, which is the correct non-promoting state because no human reviewer has recorded accept/reject notes for `battle_core` or `save_data_core`.

### 2026-04-24 - Technical Debt Audit Sprint 1: CTest Discovery Guard
- **Action**: Completed TD-AUD-01 from `docs/TECHNICAL_DEBT_AUDIT_SPRINTS_2026-04-24.md` by inspecting the generated `urpg_compat_tests-b12d07c_tests.cmake` output around the reported parse failure and confirming the current regenerated file uses valid CMake bracket arguments for the `message-text` compat test.
- **Action**: Added `tools/ci/check_ctest_discovery.ps1`, wired it into `tools/ci/run_local_gates.ps1`, and added the same guard to all three `.github/workflows/ci-gates.yml` jobs before label-specific CTest execution.
- **Validation**: Clean build reproduction path passed: `cmake --preset dev-ninja-debug -B build\dev-ninja-debug-td-aud-01-clean`, targeted test build for `urpg_tests urpg_integration_tests urpg_snapshot_tests urpg_compat_tests urpg_presentation_release_validation`, and `powershell -ExecutionPolicy Bypass -File tools\ci\check_ctest_discovery.ps1 -BuildDirectory build\dev-ninja-debug-td-aud-01-clean` discovered `1244` tests without generated CTest parse errors.
- **Validation**: Current build label lanes passed through CTest: `ctest --preset dev-all -L '^pr$' --output-on-failure` => `1162/1162`, `ctest --preset dev-all -L '^nightly$' --output-on-failure` => `35/35`, `ctest --preset dev-all -L '^weekly$' --output-on-failure` => `47/47`, and `ctest --preset dev-all -L '^regression$' --output-on-failure` => `1/1`.
- **Result**: `powershell -ExecutionPolicy Bypass -File tools\ci\run_local_gates.ps1` now passes end to end with CTest discovery checked before label lanes. The original failure is narrowed to stale/corrupted generated discovery metadata rather than a compat behavior failure or a bad Catch2 test name in the current tree.
...
- **Action**: Integrated `imports/raw/third_party_assets/huggingface/` into asset catalog, hygiene, and duplicate-planning workflows.
