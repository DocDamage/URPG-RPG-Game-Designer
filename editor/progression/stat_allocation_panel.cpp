...
nlohmann::json buildStatControlRows(const urpg::progression::StatAllocationDocument& document,
                                    ...
                                    const urpg::progression::ActorStatBlock& after,
    ...
    for (const auto& rule : pool_it->second.rules) {
        const int allocated = request.points_by_stat.count(rule.stat_id) == 0 ? 0 : request.points_by_stat.at(rule.stat_id);
        ...
        rows.push_back({{"stat_id", rule.stat_id},
                        {"before", before_value},
                        {"after", after_value},
                        ...
                        {"increment_button",
                         {{"visible", true},
                          ...
                          {"disabled_reason", can_increment ? nlohmann::json(nullptr)
                                                            : nlohmann::json(increment_disabled_reason)}}},
                        {"decrement_button",
                         {{"visible", true},
...
void StatAllocationPanel::render() {
                                   ...
                                   {{"visible", true},
                                    ...
                                    {"disabled_reason",
                                     row.can_apply ? nlohmann::json(nullptr)
                                                   : nlohmann::json(row.blocked_reason.empty() ? "blocked"
                                                                                               : row.blocked_reason)}}}});
    }
    ...
    snapshot_ = {{"panel", "stat_allocation"},
                 ...
                 {"stat_controls", buildStatControlRows(document_, request_, preview.before, preview.after, preview.remaining_points)},
                 {"controls",
                  {{"commit_button",
                    {{"visible", true},
                     ...
                     {"disabled_reason",
                      preview.diagnostics.empty() ? nlohmann::json(nullptr)
