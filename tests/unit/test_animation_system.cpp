...
TEST_CASE("AnimationSystem updates transforms", "[animation]") {
    ...
    SECTION("Entity follows animation track") {
        ...
        anim.positionTrack.push_back(
            {urpg::Fixed32::FromInt(0),
             {urpg::Fixed32::FromInt(0), urpg::Fixed32::FromInt(0), urpg::Fixed32::FromInt(0)}});
        anim.positionTrack.push_back(
    ...
    SECTION("Animation clip binding drives deterministic interpolated position") {
        ...
        clip.id = "move_right";
        clip.duration = 1.0f;
        clip.tracks = {{"positionX", {{0.0f, 0.0f}, {1.0f, 10.0f}}},
...
TEST_CASE("TimelineKernel triggers deterministic transient events", "[animation][timeline]") {
    ...
    track.events = {{0.75f, urpg::animation::TimelineEventType::TriggerVFX, "enemy_1", "slash_fx", 0.0f},
...
TEST_CASE("TimelineKernel supports scene and UI authoring workflows", "[animation][timeline]") {
    ...
    SECTION("Authoring APIs create and sort scene tracks deterministically") {
        ...
        kernel.addEvent("scene_camera",
                        {1.0f, urpg::animation::TimelineEventType::SetCamera, "camera", "close_up", 0.0f});
        kernel.addEvent("scene_camera",
                        {0.25f, urpg::animation::TimelineEventType::MoveEntity, "hero", "step_forward", 0.5f});
    ...
    SECTION("Authoring APIs update and remove UI events by stable key") {
        ...
        REQUIRE(kernel.updateEvent("ui_overlay", 0,
                                   {0.75f, urpg::animation::TimelineEventType::Dialogue, "hud", "show_prompt", 1.5f}));
