...
void WritePromotedSourceManifest(
...
std::vector<uint8_t> DecodeBundleEntryBytes(
    const std::filesystem::path& path,
    urpg::tools::ExportTarget target,
    const nlohmann::json& entry
);
std::string ComputeBundleIntegrityTag(
    const nlohmann::json& entry,
    const std::vector<uint8_t>& storedBytes,
    urpg::tools::ExportTarget target
);
std::string ComputeBundleSignature(
    const std::filesystem::path& path,
...
#ifdef _WIN32
bool LaunchProcessAndCaptureOutput(
