...
TEST_CASE("PlaytestSessionController stages a private current-map overlay", "[playtest session][editor]") {
    const auto root = temporaryProjectRoot();
    {
        ...
        const auto preview = controller.previewRedactedSupportBundle();
        REQUIRE(preview.valid);
        REQUIRE(preview.bundle["preview_required"] == true);
        REQUIRE(preview.bundle["upload_performed"] == false);
        const auto declined = controller.writeApprovedSupportBundle(preview, false);
        REQUIRE_FALSE(declined.success);
        REQUIRE_FALSE(std::filesystem::exists(session / "support" / "redacted_support_bundle.json"));
        const auto support = controller.writeApprovedSupportBundle(preview, true);
        ...
        REQUIRE(supportJson["schema"] == "urpg.redacted_support_bundle.v2");
        REQUIRE(supportJson["preview_required"] == true);
        REQUIRE(supportJson["upload_performed"] == false);
        REQUIRE(supportJson["included_sections"].size() == 5);
...
TEST_CASE("Playtest spatial bridge preserves selection through play teleport diagnostic and return",
    ...
    const auto root = temporaryProjectRoot();
    {
        ...
        REQUIRE(controller.hotReloadCurrentMap("{\"grid\":\"reloaded\"}\n", "{\"p2d\":\"reloaded\"}\n"));
        std::ifstream commands(controller.sessionDirectory()/"editor_commands.jsonl");
        std::string commandLine; REQUIRE(std::getline(commands, commandLine));
        auto command = nlohmann::json::parse(commandLine);
        REQUIRE(command["command"]=="teleport_here"); REQUIRE(command["map_id"]=="starter"); REQUIRE(command["tile_x"]==6); REQUIRE(command["tile_y"]==7);
        REQUIRE(std::getline(commands, commandLine));
        command = nlohmann::json::parse(commandLine);
        REQUIRE(command["command"] == "hot_reload_map");
        REQUIRE(command["expected_revision"] == 0);
        REQUIRE(command["state_policy"] == "reset_affected");
        std::ifstream reloadedGrid(controller.sessionDirectory()/"content"/"maps"/"starter.grid.json");
        REQUIRE(nlohmann::json::parse(reloadedGrid)["grid"] == "reloaded");
