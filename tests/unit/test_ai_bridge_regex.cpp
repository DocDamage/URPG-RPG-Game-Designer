...
TEST_CASE("Audio Bridge Regex Validation", "[ai][audio]") {
    SECTION("Standard command parsing") {
        std::string response =
            "I will play some music for you. [ACTION: PLAY_BGM, ASSET: Town_Theme, VOL: 0.8, FADE: 2.0]";
    ...
    SECTION("Command with extra whitespace and commentary") {
        std::string response =
            "Entering the dungeon... [ ACTION: CROSSFADE , ASSET: Dark_Cave , VOL: 0.5 , FADE: 5.0 ] Good luck!";
        auto cmds = urpg::ai::AudioKnowledgeBridge::parseAudioCommands(response);
    ...
    SECTION("Multiple commands") {
        std::string response =
            "[ACTION: STOP, ASSET: None, VOL: 0, FADE: 1.0] [ACTION: PLAY_SE, ASSET: Door_Open, VOL: 1.0, FADE: 0]";
        auto cmds = urpg::ai::AudioKnowledgeBridge::parseAudioCommands(response);
