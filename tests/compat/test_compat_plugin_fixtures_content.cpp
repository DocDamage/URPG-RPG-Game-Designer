...
TEST_CASE("Compat fixtures: curated codex scenarios preserve content-plugin behavior", "[compat][fixtures]") {
    ...
    REQUIRE(std::get<std::string>(std::get<Object>(defaultObject.at("topicResult").v).at("profile").v) == "quest_log");
    ...
    const Value encyclopediaResult =
        pm.executeCommand("CuratedCodexScenarioFixture", "explore", {encyclTopic, encyclRoute, encyclCategory});
    ...
    REQUIRE(std::get<std::string>(std::get<Object>(encyclopediaObject.at("boot").v).at("firstArg").v) == "ency_boot");
    ...
    const Value bookResult =
        pm.executeCommand("CuratedCodexScenarioFixture", "explore", {bookBoot, bookRoute, Value::Int(42)});
    ...
    REQUIRE(std::get<std::string>(std::get<Object>(bookObject.at("topicResult").v).at("profile").v) == "book");
...
TEST_CASE("Compat fixtures: curated codex scenarios survive plugin reload", "[compat][fixtures]") {
    PluginManager& pm = PluginManager::instance();
    pm.unloadAllPlugins();
    pm.clearFailureDiagnostics();
    ...
    const Value beforeReloadQuest = pm.executeCommand("CuratedCodexReloadFixture", "explore", {beforeReloadBoot});
    ...
    const Value afterReloadQuest = pm.executeCommandByName("CuratedCodexReloadFixture_explore", {afterReloadQuestBoot});
...
TEST_CASE("Compat fixtures: curated library-dashboard scenarios survive plugin reload", "[compat][fixtures]") {
    PluginManager& pm = PluginManager::instance();
    pm.unloadAllPlugins();
    pm.clearFailureDiagnostics();
    ...
    const Value afterReloadQuest = pm.executeCommandByName("CuratedLibraryDashboardReloadFixture_open", {questBoot});
...
TEST_CASE("Compat fixtures: curated message-text scenarios survive plugin reload", "[compat][fixtures]") {
    PluginManager& pm = PluginManager::instance();
...
TEST_CASE("Compat fixtures: curated save-data scenarios survive plugin reload", "[compat][fixtures]") {
    PluginManager& pm = PluginManager::instance();
    DataManager& data = DataManager::instance();
    pm.unloadAllPlugins();
    pm.clearFailureDiagnostics();
    ...
    const Value afterReloadAutosave = pm.executeCommand("CuratedSaveDataReloadFixture", "open",
                                                        {afterReloadAutosaveBoot, autosaveRoute, autosaveToken});
