...
void UIWindow::draw(SpriteBatcher& batcher) {
    ...
    batcher.submit(texID, m_position.x + 4.0f, m_position.y + 4.0f, m_size.x - 8.0f, m_size.y - 8.0f, bgU1, bgV1, bgU2,
                   bgV2, m_zIndex, 1.0f, 1.0f, 1.0f, m_backOpacity);
    ...
    // Corner: Top-Left
    batcher.submit(texID, m_position.x, m_position.y, 24.0f, 24.0f, 96.0f / texW, 0.0f / texH, 120.0f / texW,
                   24.0f / texH, m_zIndex + 0.01f);

    // Corner: Top-Right
    batcher.submit(texID, m_position.x + m_size.x - 24.0f, m_position.y, 24.0f, 24.0f, 168.0f / texW, 0.0f / texH,
                   192.0f / texW, 24.0f / texH, m_zIndex + 0.01f);

    // Corner: Bottom-Left
    batcher.submit(texID, m_position.x, m_position.y + m_size.y - 24.0f, 24.0f, 24.0f, 96.0f / texW, 72.0f / texH,
                   120.0f / texW, 96.0f / texH, m_zIndex + 0.01f);

    // Corner: Bottom-Right
    batcher.submit(texID, m_position.x + m_size.x - 24.0f, m_position.y + m_size.y - 24.0f, 24.0f, 24.0f, 168.0f / texW,
                   72.0f / texH, 192.0f / texW, 96.0f / texH, m_zIndex + 0.01f);
...
void UIWindow::drawGauge(SpriteBatcher& batcher, float x, float y, float width, float rate, uint32_t color1,
                         uint32_t color2) {
    ...
    batcher.submit(kSolidQuadTextureId, x, y, width, gaugeHeight, 0.0f, 0.0f, 1.0f, 1.0f, m_zIndex + 0.03f,
                   toChannel(kGaugeBackground, 24), toChannel(kGaugeBackground, 16), toChannel(kGaugeBackground, 8),
    ...
    batcher.submit(kSolidQuadTextureId, x, y, fillWidth, gaugeHeight, 0.0f, 0.0f, 1.0f, 1.0f, m_zIndex + 0.04f,
                   toChannel(color1, 24), toChannel(color1, 16), toChannel(color1, 8), toChannel(color1, 0));

    const float highlightWidth = fillWidth * 0.45f;
    if (highlightWidth > 0.0f) {
        batcher.submit(kSolidQuadTextureId, x, y, highlightWidth, gaugeHeight * 0.45f, 0.0f, 0.0f, 1.0f, 1.0f,
                       m_zIndex + 0.05f, toChannel(color2, 24), toChannel(color2, 16), toChannel(color2, 8),
                       toChannel(color2, 0));
    }
}
