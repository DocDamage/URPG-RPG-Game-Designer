...
class MenuMigration {
  public:
    ...
    static std::string MapCommandRoute(const std::string& rm_symbol) {
        ...
        if (rm_symbol == "save")
            return "scene.save";
        if (rm_symbol == "gameEnd")
            return "scene.game_end";
        if (rm_symbol == "options")
     ...
     */
    static nlohmann::json MigrateCommandPanel(const std::string& scene_id, const nlohmann::json& rm_commands,
                                              Progress& progress) {
        ...
        for (const auto& cmd : rm_commands) {
            ...
            if (cmd.contains("enabled")) {
                native_cmd["enabled_by"] = {
                    {"switch_id", ""}, {"compare", "=="}, {"value", cmd["enabled"].get<bool>()}};
            }
