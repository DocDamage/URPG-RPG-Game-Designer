...
bool IsSupportedZeroDamagePolicyValue(const std::string& value) {
    ...
    return normalized == "miss" ||
           normalized == "evasion" ||
           normalized == "evade" ||
           normalized == "zeroasevasion" ||
           normalized == "immune" ||
           normalized == "zeroasimmune" ||
           normalized == "noeffect" ||
           normalized == "none" ||
...
bool ReadImportValue(const nlohmann::json& source,
    ...
    for (auto it = source.begin(); it != source.end(); ++it) {
        ...
        if (std::find(normalized_aliases.begin(), normalized_aliases.end(), normalized_key) != normalized_aliases.end()) {
            value = it.value();
            source_key = it.key();
            return true;
        }
...
void AddImportDiagnostic(std::vector<BattleFeedbackImportDiagnostic>& diagnostics,
                         std::string code,
                         std::string message,
                         std::string target) {
...
std::optional<int32_t> ReadImportInt(const nlohmann::json& source,
                                     const std::vector<std::string>& aliases,
                                     std::string& source_key,
    ...
    AddImportDiagnostic(diagnostics,
                        "feedback_policy_value_invalid",
                        "Battle feedback fixture value must be an integer.",
                        source_key);
    return std::nullopt;
}

std::optional<bool> ReadImportBool(const nlohmann::json& source,
                                   const std::vector<std::string>& aliases,
                                   std::string& source_key,
                                   std::vector<BattleFeedbackImportDiagnostic>& diagnostics) {
    nlohmann::json value;
    ...
    AddImportDiagnostic(diagnostics,
                        "feedback_policy_value_invalid",
                        "Battle feedback fixture value must be a boolean.",
                        source_key);
    return std::nullopt;
}

std::optional<std::string> ReadImportString(const nlohmann::json& source,
                                            const std::vector<std::string>& aliases,
                                            std::string& source_key,
                                            std::vector<BattleFeedbackImportDiagnostic>& diagnostics) {
    nlohmann::json value;
    ...
    AddImportDiagnostic(diagnostics,
                        "feedback_policy_value_invalid",
                        "Battle feedback fixture value must be a string.",
                        source_key);
    return std::nullopt;
}

int32_t ClampImportedValue(int32_t value,
                           int32_t min_value,
                           int32_t max_value,
    ...
    if (clamped != value) {
        AddImportDiagnostic(diagnostics,
                            "feedback_policy_value_clamped",
                            "Battle feedback fixture value was clamped into the supported range.",
                            source_key);
    }
    return clamped;
}
...
BattleFeedbackPolicyImportResult BattleRuleResolver::importFeedbackPolicyFixture(const nlohmann::json& fixture_json) {
    ...
    const std::string fixture_name = fixture_json.is_object() ? fixture_json.value("name", "battle_feedback_fixture")
                                                             : "battle_feedback_fixture";
    ...
    if (const auto value = ReadImportInt(source,
                                         ...
                                         result.diagnostics);
        value.has_value()) {
        result.policy.chip_damage_percent =
            ClampImportedValue(*value, 0, 100, chip_damage_key, result.diagnostics);
    ...
    if (const auto value = ReadImportInt(source,
                                         ...
                                         result.diagnostics);
        value.has_value()) {
        result.policy.chip_healing_percent =
            ClampImportedValue(*value, 0, 100, chip_healing_key, result.diagnostics);
        result.imported = true;
    }

    std::string min_chip_damage_key;
    if (const auto value = ReadImportInt(source,
                                         {"minChipDamage", "min_chip_damage", "Minimum Chip Damage", "Min Chip Damage"},
                                         min_chip_damage_key,
                                         result.diagnostics);
        value.has_value()) {
    ...
    if (const auto value = ReadImportInt(source,
                                         {"minChipHealing", "min_chip_healing", "Minimum Chip Healing", "Min Chip Healing"},
                                         min_chip_healing_key,
                                         result.diagnostics);
        value.has_value()) {
    ...
    if (const auto value = ReadImportString(source,
                                            {"zeroDamagePolicy", "zero_damage_policy", "Zero Damage Presentation",
                                             "Zero Damage Policy", "Zero Damage"},
                                            zero_damage_key,
                                            result.diagnostics);
        value.has_value()) {
        if (!IsSupportedZeroDamagePolicyValue(*value)) {
            AddImportDiagnostic(result.diagnostics,
                                "feedback_policy_value_unsupported",
    ...
    result.coverage_rows.push_back(MakeCoverageRow("chip_damage", chip_damage_key.empty() ? min_chip_damage_key
                                                                                          : chip_damage_key));
    result.coverage_rows.push_back(MakeCoverageRow("chip_healing", chip_healing_key.empty() ? min_chip_healing_key
                                                                                            : chip_healing_key));
    ...
    if (result.imported) {
        result.diagnostics.insert(result.diagnostics.begin(),
                                  {"feedback_fixture_imported",
                                   "Battle feedback fixture imported with coverage evidence.",
                                   fixture_name});
    } else {
        result.diagnostics.insert(result.diagnostics.begin(),
                                  {"feedback_fixture_no_feedback_policy",
                                   "Battle feedback fixture did not contain recognized policy keys.",
                                   fixture_name});
    }
    return result;
}
