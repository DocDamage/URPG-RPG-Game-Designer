...
class BattleMigration {
  public:
                ...
                if (page.contains("list") && page["list"].is_array()) {
                    ...
                    for (size_t command_index = 0; command_index < commands.size(); ++command_index) {
                        ...
                        auto param_str = [&](size_t idx, const std::string& def) -> std::string {
                            return params.size() > idx && params[idx].is_string() ? params[idx].get<std::string>()
                                                                                  : def;
                        };
                        auto param_int = [&](size_t idx, int def) -> int {
                            return params.size() > idx && params[idx].is_number_integer() ? params[idx].get<int>()
                                                                                          : def;
                        };

                        switch (code) {
                        case 108:
                            appendUnsupportedCommandEffect(effects, progress, i, code, "comment_command", params);
                            break;
                        case 111: {
                            const auto branch_capture = captureConditionalBranch(commands, command_index);
                            appendUnsupportedCommandEffect(effects, progress, i, code, "conditional_branch", params,
                                                           branch_capture.source_commands);
                        ...
                        case 122: {
                            ...
                            if (operand_type != 0) {
                                appendUnsupportedCommandEffect(effects, progress, i, code,
                                                               "variable_operand_type_not_supported", params);
                                break;
                            }
                            nlohmann::json effect;
                        ...
                        case 408:
                            appendUnsupportedCommandEffect(effects, progress, i, code, "comment_continuation", params);
                            break;
                        case 412:
                            appendUnsupportedCommandEffect(effects, progress, i, code, "unexpected_branch_end", params);
                            break;
                        default:
                            appendUnsupportedCommandEffect(effects, progress, i, code, "unmapped_command", params);
                            break;
                        }
                    }
                }
    ...
    static nlohmann::json migrateAction(const nlohmann::json& rm_action, bool is_item, Progress& progress) {
        ...
        switch (scope) {
        case 1:
            native["scope"] = "single_enemy";
            break;
        case 2:
            native["scope"] = "all_enemies";
            break;
        case 3:
            native["scope"] = "random_enemy";
            break;
        case 4:
            native["scope"] = "random_ally";
            break;
        case 5:
            native["scope"] = "ally_dead";
            break;
        case 6:
            native["scope"] = "all_allies_dead";
            break;
        case 7:
            native["scope"] = "single_ally";
            break;
        case 8:
            native["scope"] = "all_allies";
            break;
        case 9:
            native["scope"] = "enemy_dead";
            break;
        case 10:
            native["scope"] = "all_enemies_dead";
            break;
        case 11:
            native["scope"] = "user";
            break;
        case 12:
            native["scope"] = "ally_except_user";
            break;
        default:
            native["scope"] = "none";
            progress.warnings.push_back("Battle action scope " + std::to_string(scope) +
                                        " is unsupported; falling back to 'none'.");
            break;
        }
        ...
        if (rm_action.contains("effects") && rm_action["effects"].is_array()) {
            for (const auto& effect_record : rm_action["effects"]) {
                appendUnsupportedActionEffectFallback(effect_fallbacks, progress, native["id"].get<std::string>(),
                                                      effect_record);
            }
        }
  ...
  private:
    ...
    static bool isExplicitConditionGroup(const nlohmann::json& cond) {
        return cond.is_object() && (cond.contains("children") || cond.contains("op") || cond.contains("operator") ||
    ...
    static nlohmann::json makeConditionFallback(const std::string& reason, size_t page_index,
                                                const nlohmann::json& source, const std::string& source_operator = "") {
    ...
    static void appendTypedConditionWarning(Progress& progress, size_t page_index, const std::string& reason,
                                            const std::string& detail = "") {
        std::string warning = "[battle_condition_tree_unsupported_shape] Troop page " + std::to_string(page_index) +
    ...
    static void appendUnsupportedCommandEffect(nlohmann::json& effects, Progress& progress, size_t page_index, int code,
                                               const std::string& reason, const nlohmann::json& params,
        ...
        progress.warnings.push_back("[battle_event_command_unsupported] Troop page " + std::to_string(page_index) +
    ...
    static std::string classifyActionEffectReason(const nlohmann::json& effect_record) {
        ...
        case 11:
            return "recover_hp_effect_unsupported";
        case 12:
            return "recover_mp_effect_unsupported";
        case 13:
            return "gain_tp_effect_unsupported";
        case 21:
            return "add_state_effect_unsupported";
        case 22:
            return "remove_state_effect_unsupported";
        case 31:
            return "add_buff_effect_unsupported";
        case 32:
            return "add_debuff_effect_unsupported";
        case 33:
            return "remove_buff_effect_unsupported";
        case 34:
            return "remove_debuff_effect_unsupported";
        case 41:
            return "special_effect_unsupported";
        case 42:
            return "grow_effect_unsupported";
        case 43:
            return "learn_skill_effect_unsupported";
        case 44:
            return "common_event_effect_unsupported";
        default:
            return "unmapped_effect_code";
        }
    }

    static void appendUnsupportedActionEffectFallback(nlohmann::json& fallbacks, Progress& progress,
        ...
        std::string warning =
            "[battle_action_effect_unsupported] Battle action " + native_action_id + " contains unsupported effect";
    ...
    static ConditionMigrationResult migrateConditionTree(const nlohmann::json& cond, size_t page_index,
                                                         Progress& progress) {
