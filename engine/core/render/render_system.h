...
public:
    void render(World& world, IRenderer& renderer, const CameraSystem& camera) {
        ...
        renderer.setCamera(camera.getPosition().x.ToFloat(), 
                           camera.getPosition().y.ToFloat(), 
                           1.0f); // Zoom could be added to CameraComponent

        // Draw all entities with a SpriteComponent and TransformComponent
        world.ForEachWith<TransformComponent, SpriteComponent>([&](EntityID id, TransformComponent& trans, SpriteComponent& sprite) {
            if (!sprite.visible) return;

            // Simple culling could be added here
            
            renderer.drawSprite(sprite.textureId,
                                sprite.srcX, sprite.srcY, sprite.width, sprite.height,
