...
public:
    void update(World& world, float deltaTime) {
        ...
        world.ForEachWith<TransformComponent, CameraShakeComponent>([&](EntityID, TransformComponent& transform, CameraShakeComponent& shake) {
            if (shake.trauma <= Fixed32::FromInt(0)) return;
            ...
            if (shake.mode == CameraShakeMode::Positional || shake.mode == CameraShakeMode::Both) {
                ...
                transform.position.x = transform.position.x + Fixed32::FromRaw(static_cast<int32_t>(offsetX * 65536.0f));
                transform.position.y = transform.position.y + Fixed32::FromRaw(static_cast<int32_t>(offsetY * 65536.0f));
            }
            ...
            if (shake.mode == CameraShakeMode::Rotational || shake.mode == CameraShakeMode::Both) {
                ...
                transform.rotation.z = transform.rotation.z + Fixed32::FromRaw(static_cast<int32_t>(offsetR * 65536.0f));
            }
        });
    }
