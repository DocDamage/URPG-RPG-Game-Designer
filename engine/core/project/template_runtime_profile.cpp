...
std::string containsAny(const std::string& id, const std::vector<std::string>& needles, const std::string& value) {
...
std::string genreFamilyForProfile(const TemplateRuntimeProfile& profile) {
    if (const auto family = containsAny(profile.id,
                                        {"visual_novel", "narrative", "dating", "point_and_click", "hidden_object",
                                         "puzzle_adventure", "mystery", "detective"},
                                        "narrative_adventure");
        !family.empty()) {
        return family;
    }
    if (const auto family = containsAny(profile.id,
                                        {"platformer", "metroidvania", "top_down", "action", "beat_em_up", "shmup",
                                         "twin_stick", "racing", "brick_breaker", "rhythm", "sports"},
                                        "action_arcade");
        !family.empty()) {
        return family;
    }
    if (const auto family = containsAny(profile.id,
                                        {"city_builder", "management", "tycoon", "farming", "cooking", "school", "cozy",
                                         "life_sim", "idle", "pet_shop"},
                                        "builder_management");
        !family.empty()) {
        return family;
    }
    if (const auto family = containsAny(
            profile.id, {"tactics", "tactical", "strategy", "tower_defense", "board_game", "card"}, "strategy_tactics");
        !family.empty()) {
        return family;
    }
    if (const auto family =
            containsAny(profile.id, {"roguelite", "procedural", "survival", "dungeon", "blobber", "open_world"},
                        "procedural_survival");
        !family.empty()) {
        return family;
    }
    if (const auto family =
            containsAny(profile.id, {"monster", "gacha", "collector", "tamer"}, "collection_progression");
        !family.empty()) {
        return family;
    }
    return "party_rpg";
...
std::string targetCameraForFamily(const std::string& genreFamily) {
    if (genreFamily == "narrative_adventure") {
        return "scene_framed";
    }
    if (genreFamily == "action_arcade") {
        return "side_or_top_down";
    }
    if (genreFamily == "builder_management" || genreFamily == "strategy_tactics") {
        return "top_down";
    }
    if (genreFamily == "procedural_survival") {
        return "top_down_or_first_person";
    }
    return "top_down";
}

nlohmann::json collectStarterSurfaces(const TemplateRuntimeProfile& profile) {
    nlohmann::json surfaces = nlohmann::json::array();
    if (profile.systems.contains("genre_features") && profile.systems["genre_features"].is_array()) {
        for (const auto& feature : profile.systems["genre_features"]) {
            if (surfaces.size() >= 3) {
                break;
            }
            surfaces.push_back(feature);
        }
    }
    if (surfaces.empty() && profile.systems.is_object()) {
        for (const auto& [key, value] : profile.systems.items()) {
            if (key == "accessibility" || key == "audio" || key == "localization") {
                continue;
            }
    ...
    if (surfaces.empty()) {
        surfaces = {"map_intro", "starter_menu", "save_preview"};
    }
    return surfaces;
}

nlohmann::json templateQualityContract(const TemplateRuntimeProfile& profile) {
    const auto genreFamily = genreFamilyForProfile(profile);
    return {
        {"tier", "starter_ready"},
        {"quality_score", 70},
        {"genre_family", genreFamily},
        {"target_camera", targetCameraForFamily(genreFamily)},
        {"core_loops", profile.loops},
        {"starter_content",
         {
             {"maps_or_scenes", 3},
             {"sample_entities", 4},
             {"sample_progression_hooks", 2},
             {"wysiwyg_surfaces", collectStarterSurfaces(profile)},
         }},
        ...
        {"localization_namespaces", {profile.id}},
        {"performance_budget", {{"frame_time_ms", 33.3}, {"arena_kb", 512}, {"active_entities", 128}}},
        {"export_profile", "standalone_strict"},
        {"known_limits",
         {"bounded starter content only", "no platform-holder certification", "no online services or live economy"}},
    };
}
                 ...
                 {"deploymentZones",
                  {
                      {{"id", "player_start"},
                       {"team", "player"},
                       {"cells", {{{"x", 1}, {"y", 5}}, {{"x", 1}, {"y", 6}}}}},
                      {{"id", "enemy_start"},
                       {"team", "enemy"},
                       {"cells", {{{"x", 10}, {"y", 4}}, {{"x", 10}, {"y", 5}}}}},
                  }},
                 {"turnOrder", "speed_then_team_priority"},
                 {"winConditions", {"defeat_all_enemies", "survive_turns"}},
             }},
            {"accessibility",
             {{"focusLabels", {"unit_roster", "grid_cursor", "action_menu"}}, {"contrastMode", "tactics_grid"}}},
            {"audio", {{"cues", {"unit_select", "move_confirm", "attack_preview", "turn_start"}}}},
            {"localization",
             {{"requiredKeys", {"scenario.title", "objective.primary", "action.move", "action.attack"}}}},
        },
    };
}
...
TemplateRuntimeProfile makeArpgProfile() {
    return {
        ...
        readyBars(
            {"arpg_combat_labels", "realtime_audio_cues", "action_input_map", "combat_text_keys", "frame_budget_16ms"}),
        {
            {"combat",
             {
...
TemplateRuntimeProfile makeMonsterCollectorProfile() {
    return {
        ...
        readyBars({"collection_roster_labels", "capture_audio_cues", "party_input_map", "creature_text_keys",
                   "encounter_budget"}),
        {
            {"collection",
             {
                 ...
                 {"capture",
                  {{"formula", "base_rate + status_bonus - rarity_penalty"},
                   {"items", {"basic_charm", "strong_charm"}}}},
             }},
            {"accessibility",
             {{"requiredLabels", {"creature_roster", "capture_button", "party_slot", "ability_grid"}}}},
...
TemplateRuntimeProfile makeCozyLifeProfile() {
    return {
        ...
        readyBars(
            {"schedule_labels", "ambient_audio_cues", "life_sim_input_map", "social_text_keys", "day_tick_budget"}),
        {
            {"schedule",
             {
...
TemplateRuntimeProfile makeMetroidvaniaProfile() {
    return {
        ...
        readyBars(
            {"traversal_labels", "ability_audio_cues", "platforming_input_map", "region_text_keys", "room_budget"}),
        {
            {"traversal",
             {
                 {"abilities",
            ...
            {"localization",
             {{"requiredKeys", {"ability.dash", "ability.wall_jump", "region.locked", "map.checkpoint"}}}},
        },
    };
}
...
TemplateRuntimeProfile makeSurvivalHorrorProfile() {
    return {
        ...
        readyBars({"horror_readability_labels", "tension_audio_cues", "resource_input_map", "puzzle_text_keys",
                   "lighting_budget"}),
        {
            {"tension",
             {
            ...
            {"accessibility",
             {{"requiredLabels", {"flashlight_meter", "inventory_slots", "safe_room_prompt", "threat_warning"}}}},
...
TemplateRuntimeProfile makePlatformerRpgProfile() {
    return {
        ...
        readyBars(
            {"platformer_labels", "jump_audio_cues", "platformer_input_map", "checkpoint_text_keys", "physics_budget"}),
        {
            {"platformer",
             {
            ...
            {"localization",
             {{"requiredKeys", {"platform.jump", "checkpoint.saved", "hazard.warning", "ability.dash"}}}},
        },
    };
}
...
TemplateRuntimeProfile makeGachaHeroProfile() {
    return {
        ...
        readyBars({"summon_labels", "summon_audio_cues", "roster_input_map", "banner_text_keys", "simulation_budget"}),
        {
            ...
            {"accessibility",
             {{"requiredLabels", {"summon_button", "rate_details", "pity_counter", "hero_roster_slot"}}}},
...
TemplateRuntimeProfile makeMysteryDetectiveProfile() {
    return {
        ...
        readyBars(
            {"clue_labels", "investigation_audio_cues", "case_input_map", "deduction_text_keys", "case_graph_budget"}),
        {
            {"casework",
             {
            ...
            {"accessibility",
             {{"requiredLabels", {"clue_card", "case_thread", "evidence_prompt", "deduction_result"}}}},
...
TemplateRuntimeProfile makeWorldExplorationProfile() {
    return {
        ...
        {"presentation_runtime", "message_text_core", "save_data_core", "gameplay_ability_framework",
         "export_validator"},
...
TemplateRuntimeProfile makeGenreProfile(std::string id, std::string displayName, std::vector<std::string> tags,
                                        std::vector<std::string> loops, std::string primarySystem,
    ...
    return {
                   ...
                   prefix + "_budget"}),
        {
            ...
            {"accessibility",
             {{"requiredLabels", {prefix + "_primary", prefix + "_status", prefix + "_action", prefix + "_warning"}}}},
            {"audio", {{"cues", {prefix + "_open", prefix + "_confirm", prefix + "_success", prefix + "_warning"}}}},
            {"localization",
             {{"requiredKeys", {prefix + ".title", prefix + ".action", prefix + ".status", prefix + ".warning"}}}},
        },
    };
}

TemplateRuntimeProfile makeNonRpgProfile(std::string id, std::string displayName, std::vector<std::string> tags,
                                         std::vector<std::string> loops, std::string primarySystem,
                                         std::vector<std::string> featureList) {
    const std::string prefix = id;
    return {
        ...
        "READY",
        std::move(tags),
        {"ui_menu_core", "message_text_core", "save_data_core", "presentation_runtime", "export_validator"},
        std::move(loops),
        readyBars({prefix + "_labels", prefix + "_audio_cues", prefix + "_input_map", prefix + "_text_keys",
                   prefix + "_budget"}),
        {
...
TemplateRuntimeProfile makeTacticalMechaProfile() {
    return makeGenreProfile("tactical_mecha_rpg", "Tactical Mecha RPG", {"mecha", "tactics", "loadouts"},
                            {"mech_loadout_loop", "grid_battle_loop", "pilot_bond_loop", "save_loop"}, "mecha",
                            {"parts_loadout", "heat_ammo", "pilot_bonds", "grid_deployment"});
}
...
TemplateRuntimeProfile makeMonsterTamerArenaProfile() {
    return makeGenreProfile("monster_tamer_arena", "Monster Tamer Arena", {"monster", "arena", "tournament"},
                            {"team_build_loop", "arena_battle_loop", "evolution_training_loop", "save_loop"}, "arena",
                            {"team_rules", "league_ladder", "breeding", "evolution"});
}
...
TemplateRuntimeProfile makeIdleIncrementalProfile() {
    return makeGenreProfile("idle_incremental_rpg", "Idle Incremental RPG", {"idle", "prestige", "automation"},
                            {"offline_gain_loop", "auto_battle_loop", "prestige_upgrade_loop", "save_loop"}, "idle",
                            {"offline_gains", "prestige", "auto_battle", "upgrade_tree"});
}
...
TemplateRuntimeProfile makeStrategyKingdomProfile() {
    return makeGenreProfile("strategy_kingdom_rpg", "Strategy Kingdom RPG", {"kingdom", "resources", "armies"},
                            {"kingdom_build_loop", "recruitment_loop", "army_mission_loop", "save_loop"}, "kingdom",
                            {"buildings", "recruiting", "resources", "army_missions"});
}
...
TemplateRuntimeProfile makeRhythmRpgProfile() {
    return makeGenreProfile("rhythm_rpg", "Rhythm RPG", {"rhythm", "combat", "timing"},
                            {"song_chart_loop", "timed_battle_loop", "performance_grade_loop", "save_loop"}, "rhythm",
                            {"beat_timed_combat", "song_charts", "timing_windows", "grades"});
}

TemplateRuntimeProfile makeCookingRestaurantProfile() {
    return makeGenreProfile("cooking_restaurant_rpg", "Cooking Restaurant RPG",
                            {"cooking", "restaurant", "relationships"},
...
TemplateRuntimeProfile makePirateProfile() {
    return makeGenreProfile("pirate_rpg", "Pirate RPG", {"pirates", "crew", "islands"},
                            {"crew_ship_loop", "island_treasure_loop", "naval_encounter_loop", "save_loop"}, "pirate",
                            {"ship_upgrades", "crew", "treasure_maps", "naval_combat"});
}

TemplateRuntimeProfile makeSportsTeamProfile() {
    return makeGenreProfile("sports_team_rpg", "Sports Team RPG", {"sports", "team", "season"},
                            {"team_roster_loop", "training_week_loop", "match_day_loop", "save_loop"}, "sportsTeam",
                            {"team_roster", "training", "season_matches", "relationships"});
}
...
TemplateRuntimeProfile makePetShopProfile() {
    return makeGenreProfile("pet_shop_creature_care_rpg", "Pet Shop Creature Care RPG", {"pets", "care", "shop"},
                            {"pet_care_loop", "breeding_loop", "customer_request_loop", "save_loop"}, "petCare",
                            {"pet_care", "breeding", "shop_economy", "customer_requests"});
}
...
TemplateRuntimeProfile makeDetectiveNoirProfile() {
    return makeGenreProfile("detective_noir_vn_rpg", "Detective Noir VN RPG", {"noir", "visual-novel", "evidence"},
                            ...
                            "detectiveNoir",
                            {"evidence_boards", "branching_endings", "noir_dialogue", "case_variables"});
}
...
TemplateRuntimeProfile makeOpenWorldSurvivalProfile() {
    return makeGenreProfile("open_world_survival_rpg", "Open World Survival RPG",
...
TemplateRuntimeProfile makeFactionPoliticsProfile() {
    return makeGenreProfile(
        "faction_politics_rpg", "Faction Politics RPG", {"factions", "diplomacy", "territory"},
...
TemplateRuntimeProfile makeBasicProfile(std::string id, std::string displayName, std::vector<std::string> tags,
                                        std::vector<std::string> subsystems, std::vector<std::string> loops) {
    return {std::move(id),
            std::move(displayName),
            "READY",
            std::move(tags),
            std::move(subsystems),
            std::move(loops),
            readyBars({"baseline_accessibility", "baseline_audio", "baseline_input", "baseline_localization",
                       "baseline_perf"}),
...
std::vector<TemplateRuntimeProfile> allTemplateRuntimeProfiles() {
    return {
        makeBasicProfile("jrpg", "Classic JRPG", {"party", "battle", "exploration"},
                         {"ui_menu_core", "message_text_core", "battle_core", "save_data_core"},
                         {"battle_loop", "save_loop"}),
        makeBasicProfile("visual_novel", "Visual Novel", {"dialogue", "choices", "save"},
                         {"message_text_core", "save_data_core"}, {"dialogue_loop", "save_loop"}),
        makeBasicProfile("turn_based_rpg", "Turn-Based RPG", {"tactics", "battle", "progression"},
        ...
        makeGenreProfile("western_crpg", "Western CRPG", {"crpg", "party", "quests", "choices"},
                         {"quest_dialogue_loop", "companion_party_loop", "reputation_choice_loop", "save_loop"},
                         "westernCrpg", {"companions", "dialogue_trees", "reputation", "tactical_encounters"}),
        makeGenreProfile("dungeon_crawler_rpg", "Dungeon Crawler RPG", {"dungeon", "party", "mapping"},
                         {"grid_exploration_loop", "trap_encounter_loop", "party_resource_loop", "save_loop"},
                         "dungeonCrawler", {"automap", "traps", "encounter_tables", "party_resources"}),
        makeGenreProfile("sandbox_crafting_rpg", "Sandbox Crafting RPG", {"crafting", "sandbox", "equipment"},
                         {"gathering_loop", "recipe_crafting_loop", "station_upgrade_loop", "save_loop"},
                         "sandboxCrafting", {"resources", "recipes", "crafting_stations", "equipment_progression"}),
        makeGenreProfile("life_sim_rpg", "Life Sim RPG", {"life-sim", "relationships", "calendar"},
                         {"schedule_day_loop", "relationship_event_loop", "town_routine_loop", "save_loop"}, "lifeSim",
                         {"schedules", "relationships", "jobs", "calendar_events"}),
        makeGenreProfile("mmorpg_lite", "MMORPG Lite", {"mmo-lite", "quests", "zones"},
                         {"quest_zone_loop", "cooldown_role_loop", "loot_vendor_loop", "save_loop"}, "mmorpgLite",
                         {"zones", "quests", "role_abilities", "vendors"}),
        makeGenreProfile("action_adventure_rpg", "Action Adventure RPG", {"action-adventure", "dungeons", "items"},
                         {"dungeon_item_loop", "puzzle_gate_loop", "exploration_combat_loop", "save_loop"},
                         "actionAdventure", {"dungeons", "key_items", "puzzles", "heart_upgrades"}),
        makeGenreProfile("blobber_rpg", "Blobber RPG", {"blobber", "grid", "party"},
                         {"grid_party_exploration_loop", "encounter_table_loop", "front_back_row_loop", "save_loop"},
                         "blobber", {"grid_movement", "automap", "rows", "encounter_tables"}),
        makeGenreProfile("deckbuilding_roguelite_rpg", "Deckbuilding Roguelite RPG",
                         {"deckbuilding", "roguelite", "cards"},
                         {"run_map_loop", "deck_reward_loop", "relic_build_loop", "save_loop"}, "deckbuildingRoguelite",
                         {"route_map", "relics", "deck_drafts", "escalating_encounters"}),
        makeGenreProfile("colony_survival_rpg", "Colony Survival RPG", {"colony", "survival", "jobs"},
                         {"colony_job_loop", "crafting_raid_loop", "character_need_loop", "save_loop"},
                         "colonySurvival", {"jobs", "colonist_stats", "raids", "base_crafting"}),
        makeGenreProfile("merchant_trader_rpg", "Merchant Trader RPG", {"trade", "economy", "routes"},
                         {"trade_route_loop", "market_pricing_loop", "caravan_upgrade_loop", "save_loop"},
                         "merchantTrader", {"markets", "routes", "reputation", "caravan_upgrades"}),
        makeGenreProfile("dating_sim_rpg", "Dating Sim RPG", {"dating-sim", "relationships", "endings"},
                         {"affection_event_loop", "schedule_choice_loop", "ending_route_loop", "save_loop"},
                         "datingSim", {"affection", "schedules", "date_events", "endings"}),
        makeGenreProfile("procedural_world_rpg", "Procedural World RPG", {"procedural", "world", "quests"},
                         {"seed_world_loop", "generated_quest_loop", "town_dungeon_loop", "save_loop"},
                         "proceduralWorld",
                         {"world_seed", "generated_towns", "procedural_dungeons", "quest_templates"}),
        makeGenreProfile("isekai_party_rpg", "Isekai Party RPG", {"isekai", "party", "guild"},
                         {"guild_quest_loop", "party_recruit_loop", "portal_story_loop", "save_loop"}, "isekaiParty",
                         {"portal_origin", "guild_quests", "party_recruitment", "fantasy_jobs"}),
        makeGenreProfile("boss_rush_rpg", "Boss Rush RPG", {"boss-rush", "builds", "unlocks"},
                         {"boss_sequence_loop", "build_upgrade_loop", "unlock_reward_loop", "save_loop"}, "bossRush",
                         {"boss_phases", "builds", "upgrades", "unlocks"}),
        makeGenreProfile("tactical_card_rpg", "Tactical Card RPG", {"tactics", "cards", "grid"},
                         {"grid_card_turn_loop", "hand_ability_loop", "draft_loadout_loop", "save_loop"},
                         "tacticalCard", {"grid_tactics", "cards", "hand_management", "loadouts"}),
        makeNonRpgProfile("point_and_click_adventure", "Point and Click Adventure",
                          {"adventure", "hotspots", "inventory"},
                          {"room_hotspot_loop", "inventory_puzzle_loop", "dialogue_scene_loop", "save_loop"},
                          "pointAndClick", {"hotspots", "inventory_items", "item_use", "scene_transitions"}),
        makeNonRpgProfile("hidden_object_adventure", "Hidden Object Adventure", {"hidden-object", "adventure", "hints"},
                          {"object_search_loop", "hint_recharge_loop", "scene_goal_loop", "save_loop"}, "hiddenObject",
                          {"object_lists", "hints", "timed_search", "scene_goals"}),
        makeNonRpgProfile("puzzle_adventure", "Puzzle Adventure", {"puzzle", "adventure", "flags"},
                          {"room_puzzle_loop", "switch_key_loop", "progression_flag_loop", "save_loop"},
                          "puzzleAdventure", {"room_puzzles", "switches", "keys", "progression_flags"}),
        makeNonRpgProfile("narrative_adventure", "Narrative Adventure", {"narrative", "choices", "branches"},
                          {"choice_scene_loop", "branch_state_loop", "ending_resolution_loop", "save_loop"},
                          "narrativeAdventure", {"choices", "branching_state", "scenes", "endings"}),
        makeNonRpgProfile("platformer", "Platformer", {"platformer", "movement", "collectibles"},
                          {"movement_checkpoint_loop", "hazard_collectible_loop", "stage_goal_loop", "save_loop"},
                          "platformer", {"movement", "collision", "hazards", "checkpoints"}),
        makeNonRpgProfile("precision_platformer", "Precision Platformer", {"platformer", "precision", "timers"},
                          {"retry_timer_loop", "hazard_medal_loop", "checkpoint_loop", "save_loop"},
                          "precisionPlatformer", {"retries", "timers", "hazards", "level_medals"}),
        makeNonRpgProfile("top_down_action", "Top Down Action", {"action", "top-down", "arenas"},
                          {"arena_combat_loop", "pickup_weapon_loop", "stage_progression_loop", "save_loop"},
                          "topDownAction", {"movement", "attacks", "enemies", "pickups"}),
        makeNonRpgProfile("beat_em_up", "Beat Em Up", {"brawler", "lanes", "combos"},
                          {"lane_brawler_loop", "wave_combo_loop", "pickup_stage_loop", "save_loop"}, "beatEmUp",
                          {"lanes", "waves", "combos", "pickups"}),
        makeNonRpgProfile("tower_defense", "Tower Defense", {"tower-defense", "waves", "lanes"},
                          {"wave_lane_loop", "tower_upgrade_loop", "enemy_path_loop", "save_loop"}, "towerDefense",
                          {"waves", "towers", "lanes", "upgrades"}),
        makeNonRpgProfile("city_builder", "City Builder", {"city", "zoning", "resources"},
                          {"zoning_build_loop", "service_need_loop", "resource_flow_loop", "save_loop"}, "cityBuilder",
                          {"zoning", "resources", "placement", "services"}),
        makeNonRpgProfile("management_sim", "Management Sim", {"management", "staff", "queues"},
                          {"staff_room_loop", "queue_customer_loop", "finance_upgrade_loop", "save_loop"},
                          "managementSim", {"staff", "rooms", "queues", "finances"}),
        makeNonRpgProfile("tycoon_sim", "Tycoon Sim", {"tycoon", "economy", "expansion"},
                          {"build_price_loop", "optimize_expand_loop", "profit_upgrade_loop", "save_loop"}, "tycoonSim",
                          {"buildings", "pricing", "optimization", "expansion"}),
        makeNonRpgProfile("idle_incremental", "Idle Incremental", {"idle", "incremental", "prestige"},
                          {"generator_upgrade_loop", "automation_loop", "prestige_loop", "save_loop"},
                          "idleIncremental", {"generators", "prestige", "automation", "offline_gains"}),
        makeNonRpgProfile("match_3_puzzle", "Match 3 Puzzle", {"match-3", "puzzle", "boosters"},
                          {"board_match_loop", "cascade_goal_loop", "booster_level_loop", "save_loop"}, "match3",
                          {"board_matching", "cascades", "goals", "boosters"}),
        makeNonRpgProfile("falling_block_puzzle", "Falling Block Puzzle", {"falling-block", "puzzle", "scoring"},
                          {"piece_drop_loop", "line_clear_loop", "level_score_loop", "save_loop"}, "fallingBlock",
                          {"pieces", "board_logic", "scoring", "levels"}),
        makeNonRpgProfile("brick_breaker", "Brick Breaker", {"arcade", "paddle", "physics"},
                          {"paddle_ball_loop", "brick_powerup_loop", "stage_clear_loop", "save_loop"}, "brickBreaker",
                          {"paddle", "ball_physics", "bricks", "powerups"}),
        makeNonRpgProfile("shmup", "Shmup", {"shooter", "scrolling", "bosses"},
                          {"scroll_wave_loop", "powerup_weapon_loop", "boss_stage_loop", "save_loop"}, "shmup",
                          {"waves", "bullets", "powerups", "bosses"}),
        makeNonRpgProfile("twin_stick_shooter", "Twin Stick Shooter", {"shooter", "arena", "weapons"},
                          {"arena_wave_loop", "aim_weapon_loop", "pickup_survival_loop", "save_loop"}, "twinStick",
                          {"arena_movement", "aiming", "waves", "weapons"}),
        makeNonRpgProfile("racing_top_down", "Top Down Racing", {"racing", "tracks", "time-trial"},
                          {"lap_checkpoint_loop", "handling_drift_loop", "time_trial_loop", "save_loop"},
                          "topDownRacing", {"tracks", "laps", "checkpoints", "handling"}),
        makeNonRpgProfile("rhythm_game", "Rhythm Game", {"rhythm", "charts", "scoring"},
                          {"chart_timing_loop", "score_grade_loop", "song_unlock_loop", "save_loop"}, "rhythmGame",
                          {"charts", "timing_windows", "scoring", "song_unlocks"}),
        makeNonRpgProfile("sports_arcade", "Sports Arcade", {"sports", "arcade", "matches"},
                          {"match_play_loop", "team_stat_loop", "season_round_loop", "save_loop"}, "sportsArcade",
                          {"matches", "stats", "simple_ai", "teams"}),
        makeNonRpgProfile("board_game_digital", "Digital Board Game", {"board-game", "turns", "local-multiplayer"},
                          {"turn_order_loop", "piece_card_loop", "rule_resolution_loop", "save_loop"}, "boardGame",
                          {"turn_order", "pieces", "cards", "local_multiplayer"}),
        makeNonRpgProfile("party_minigame_collection", "Party Minigame Collection",
                          {"party", "minigames", "local-multiplayer"},
                          {"minigame_round_loop", "scoreboard_loop", "local_player_loop", "save_loop"},
                          "partyMinigames", {"minigames", "scoring", "rounds", "local_multiplayer"}),
        makeNonRpgProfile("educational_quiz", "Educational Quiz", {"quiz", "education", "categories"},
                          {"question_answer_loop", "category_score_loop", "feedback_progress_loop", "save_loop"},
                          "educationalQuiz", {"questions", "categories", "scoring", "feedback"}),
    };
}
...
std::optional<TemplateRuntimeProfile> findTemplateRuntimeProfile(const std::string& templateId) {
    ...
    const auto it = std::find_if(profiles.begin(), profiles.end(),
                                 [&](const TemplateRuntimeProfile& profile) { return profile.id == templateId; });
    if (it == profiles.end()) {
        return std::nullopt;
    }
    return *it;
