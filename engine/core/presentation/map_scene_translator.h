...
public:
    static const SpatialMapOverlay* ResolveMapOverlay(
    ...
    static const ActorPresentationProfile* ResolveActorProfile(
        const PresentationAuthoringData& data,
     ...
     */
    static Vec3 CalculateActorWorldPosition(
        ...
        float height = elevation.GetWorldHeight(static_cast<uint32_t>(ix >= 0 ? ix : 0), 
                                                static_cast<uint32_t>(iy >= 0 ? iy : 0));
    ...
    void Translate(
        ...
        for (const auto& actor : sceneState.actors) {
            ...
            visibleActorGroundPositions.push_back({actor.actorId, CalculateActorWorldPosition(actor, mapData->elevation)});
        ...
        for (size_t index = 0; index < mapData->props.size(); ++index) {
            ...
            outIntent.AddProp(static_cast<uint32_t>(index + 1), Vec3{prop.posX, prop.posY, prop.posZ}, Vec3{0, prop.rotY, 0});
        }
