...
public:
    static LODLevel ResolveLOD(float distance, const LODPolicy& policy, CapabilityTier tier) {
        ...
        if (distance > policy.cullDistance * multiplier) return LODLevel::LOD3_Culled;
        if (distance > policy.lod2Distance * multiplier) return LODLevel::LOD2_Low;
        if (distance > policy.lod1Distance * multiplier) return LODLevel::LOD1_Medium;
