...
nlohmann::json makePolicyRow(std::string id, std::string label, std::string status, std::string provider,
    ...
    return {{"id", std::move(id)},
            ...
            {"missingCredential", std::move(missingCredential)},
            {"evidencePath", std::move(evidencePath)},
...
nlohmann::json ExportValidator::buildPlatformArtifactPolicy(const std::string& path, tools::ExportTarget target) const {
    ...
    const bool smokeEvidenceExists = std::filesystem::exists(root / "smoke_evidence.json");
    const bool signingReady = envEnabled("URPG_SIGNING_PROVIDER_READY");
    const bool notarizationReady =
    ...
    rows.push_back(makePolicyRow("launched_smoke", "Launched package smoke",
                                 smokeEvidenceExists ? "evidence_present" : "missing_evidence", "local_launcher",
                                 smokeEvidenceExists ? "" : "smoke_evidence.json",
                                 (root / "smoke_evidence.json").generic_string(), true));

    std::size_t releaseBlockingCount = 0;
    for (const auto& row : rows) {
        if (!row.value("requiredForRelease", false)) {
            continue;
        }
        const auto status = row.value("status", "");
        if (status == "missing_credentials" || status == "missing_evidence") {
...
nlohmann::json ExportValidator::buildReportJson(const std::string& path, tools::ExportTarget target) const {
    ...
    report["platformArtifactPolicy"] = buildPlatformArtifactPolicy(path, target);
