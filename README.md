...
- 3D Dungeon World support with RPG Maker-style 2D-map-to-3D switching, raycast runtime preview, wall/floor/ceiling material customization with lighting and footstep metadata, collision-aware movement, facing-cell interaction/stair/event preview, locked doors, required items, secret walls, floor transfers, encounter triggers, persistent automap/session state, minimap visibility, diagnostics, saved JSON, and a release-registered WYSIWYG editor panel.
